...
# Plot
ggplot(nest_survival_reals_status_table, aes(x = region, y = Hatching_rate, fill = fill_col)) +
  geom_col(position = position_dodge(width = 0.8), color = "black", width = 0.7) +
  scale_fill_identity() +  # use the colours as-is
  ylim(0, 100) +
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data <- BSC_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
BSC_chicks_Phi_run$model.table %>% 
  dplyr::select(model, npar, DeltaAICc, weight) %>% gt() %>% 
  cols_label(model = html("<i>Bellarine_Surf Coast</i>"), 
             npar = "K", DeltaAICc = "DeltaAICc", weight = "weight") %>% 
  fmt_number(columns = DeltaAICc, rows = 1:10, decimals = 2, use_seps = FALSE) %>% 
  fmt_number(columns = weight, rows = 1:10, decimals = 3, use_seps = FALSE) %>% 
  cols_align(align = "left", columns = vars(model)) %>% 
