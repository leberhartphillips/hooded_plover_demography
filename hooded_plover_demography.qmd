...
hatch_dates_BSC_MP_FP_BC_PI <- 
  # NOTE: for MP need to use nest_ID "1" for chick data merge
...
MP_chick_data <-
  suppressMessages(
    ...
    dplyr::select(region, season, site, nest_ID, date, n_adults_seen, n_juvs_seen, 
                  n_chicks_seen, habitat, chick_age, management_type, lat, lon) %>%
    left_join(., hatch_dates_BSC_MP_FP %>% select(nest_ID, hatch_date), by = "nest_ID")

### NOTE: proritise Nest data (clutch completion date)
...
MP_chick_data_clean_hatch_dates <- 
  ...
  filter(hatch_date == hatch_date2 | (is.na(hatch_date) & !is.na(hatch_date2)))

### NOTE: if nest based hatch dates do not match chick based dates, then use nest hatch dates
...
MP_chick_data %>%
  ...
  filter(hatch_date_diff != 0 | is.na(hatch_date_diff) | (hatch_date == hatch_date2))

MP_chick_data %>%
  left_join(., na_hatch_dates, by = "nest_ID") %>% 
  select(nest_ID, hatch_date, hatch_date2, chick_age, date) %>% 
  distinct() %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date%>%
  group_by(nest_ID) %>%
  ...
  mutate(hatch_date_diff = ifelse(is.na(hatch_date_diff), 0, hatch_date_diff))  %>% 
  arrange(desc(abs(hatch_date_diff))) %>%
  pull(hatch_date_diff) %>%
  as.numeric() %>%
  hist(main = "Distribution of hatch_date_diff",
...
MP_chick_data_issues <- 
  ...
  mutate(wardens = ifelse(str_detect(management_type, "wardens"), 1, 0)) %>% 
  mutate(none = ifelse(str_detect(management_type, "none"), 1, 
                       ifelse(is.na(management_type), 1, 0))) %>%
  mutate(other = ifelse(str_detect(management_type, "other"), 1, 0)) %>%
  mutate(management_level = ifelse((sign_access == 1 | perm_fence == 1) & sign_nest == 1 & rope_fence == 1 & wardens == 1, 4,
                                   ifelse(rope_fence == 1, 3,
                                          ifelse(sign_nest == 1, 2,
...
```{r}
### NOTE: use the management status and type from the Nest data (see columns "Chick manged?" and "Chick management type")
...
# this one had dynamic management for example
MP_chick_data_clean %>% 
  filter(nest_ID == "201213_Koonya East_3") %>% View()
```
...
Chick_p_reals <- MP_chicks_p_run[[8]]$results$real
...
Chick_dot_surv <- MP_chicks_Phi_run[[21]]$results$real
Chick_quadage_surv <- MP_chicks_Phi_run[[26]]$results$real
Chick_hatch_date_surv <- MP_chicks_Phi_run[[10]]$results$real
Chick_mgmt_s_surv <- MP_chicks_Phi_run[[23]]$results$real
