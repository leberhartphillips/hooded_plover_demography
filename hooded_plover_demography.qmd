...
funday_summary_MP <- MP_chick_data_clean %>%
  rowwise() %>% 
  mutate(
    weekend_days = ifelse(
...
MP_chicks_Phi_run # best structure: Phi(~season * Quadratic)
...
```{r}
# Create processed RMARK data format as CJS with "Year" as group
BC_chicks.proc <- process.data(BC_chick_ch, model = "CJS",
                               groups = c("season", "region", 
                                          "mgmt_l", "mgmt_s"))

# Create the design data
BC_chicks.ddl <- make.design.data(BC_chicks.proc)

# create a quadratic time variable
time <- c(0:(BC_chicks.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(BC_chicks.proc$nocc[1]-1),
                         funday = BC_chick_ch$funday[1:BC_chicks.proc$nocc[1]])
BC_chicks.ddl$p <- merge_design.covariates(BC_chicks.ddl$Phi, 
                                              quad_time, bygroup = FALSE, bytime = TRUE)
BC_chicks.ddl$Phi <- merge_design.covariates(BC_chicks.ddl$Phi, 
                                                quad_time, bygroup = FALSE, bytime = TRUE)
BC_chicks.ddl$Phi <- merge_design.covariates(BC_chicks.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```
...
BC_chicks_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = BC_chicks.proc, 
                             ddl = BC_chicks.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
...
# Run model selection to determine best structure of p
BC_chicks_p_run <- BC_chicks_p()

# inspect p AIC table to find top model
BC_chicks_p_run # best structure: p(~1) #model 3

saveRDS(BC_chicks_p_run, "output/BC_chicks_p_run.rds")

#Create AIC model selection table

BC_chicks_p_run <- readRDS(file = "output/BC_chicks_p_run.rds") 

BC_chicks_p_run$model.table %>% 
  mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "time",
...
# Create processed RMARK data format as CJS with "Year" as group
chicks.proc <- process.data(chick_ch, model = "CJS",
                               groups = c("season", "region", 
                                          "mgmt_l", "mgmt_s"))

# Create the design data
chicks.ddl <- make.design.data(chicks.proc)

# create a quadratic time variable
time <- c(0:(chicks.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(chicks.proc$nocc[1]-1),
                         funday = chick_ch$funday[1:chicks.proc$nocc[1]])
chicks.ddl$p <- merge_design.covariates(chicks.ddl$Phi, 
                                              quad_time, bygroup = FALSE, bytime = TRUE)
chicks.ddl$Phi <- merge_design.covariates(chicks.ddl$Phi, 
                                                quad_time, bygroup = FALSE, bytime = TRUE)
chicks.ddl$Phi <- merge_design.covariates(chicks.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```
...
chicks_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = chicks.proc, 
                             ddl = chicks.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
...
# Run model selection to determine best structure of p
chicks_p_run <- chicks_p()

# inspect p AIC table to find top model
chicks_p_run # best structure: p(~season + date_j)
...
chicks_Phi <- function() 
{
  ...
  # Additive effect of region and management level
  Phi.region.mgmt_l <-
    list(formula = ~ region + mgmt_l)

  # Additive effect of region and management status
  Phi.region.mgmt_s <-
...
```{r}
tbl_reg
ression(humans_fun_day_BC_, intercept = TRUE, 
               label = list(obs_date2 ~ "Date", funday ~ "Weekends & Holidays"))
```
