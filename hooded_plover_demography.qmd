...
# Save the workbook
saveWorkbook(BC_nest_issues, here("data/nest_issues_commented/BC_nest_data_issues_220825.xlsx"), overwrite = TRUE)
```
...
# Save the workbook
saveWorkbook(PI_nest_issues, here("data/nest_issues_commented/PI_nest_data_issues_220825.xlsx"), overwrite = TRUE)
```
...
threat_data <- 
  bind_rows(FP_threat_data, MP_threat_data, BC_threat_data, BC_threat_data, PI_threat_data)
```
###Import threat data
```{r, eval=FALSE}
threat_data_ <-
...
sites_intersect_PI <- 
  inner_join(nest_data_BC, threat_data__ %>% filter(region == "PI"), by = c("season", "site"), relationship = "many-to-many") %>% 
  dplyr::select(season, site) %>% distinct() %>% 
  mutate(season_site = paste(season, site, sep = "_"))
...
saveRDS(nest_survival_BC, file = "output/nest_survival_BC_threats&status_220825.rds")
```
...
nest_survival_BC <- readRDS(file = "output/nest_survival_BC_threats&status_220825.rds")
...
pred.fundays <- 
  bind_rows(pred.fundays_FP, pred.fundays_MP, pred.fundays_BC)
...
pred.fundays_status <- 
  bind_rows(pred.fundays_status_FP, pred.fundays_status_MP, pred.fundays_status_BC)
