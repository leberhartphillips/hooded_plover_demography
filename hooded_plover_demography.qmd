...
MP_chick_data_clean <- MP_chick_data %>%
  mutate(
...
MP_chick_data_clean <- MP_chick_data_clean %>%
  left_join(funday_summary_MP, by = "nest_ID", relationship = "many-to-many")

```
```{r}
##BC
# Find brood start and end date
BC_chick_data_clean <- BC_chick_data %>%
  mutate(
    date_formatted = as.Date(date, format = "%d%m%Y")  # new column with proper Date format
  ) %>%
  group_by(nest_ID) %>%
...
# Step 3: Join funday count back to every row
BC_chick_data_clean <- BC_chick_data_clean %>%
  left_join(funday_summary_BC, by = "nest_ID", relationship = "many-to-many")

```
```{r}
## BSC
# Find brood start and end date
BSC_chick_data_clean <- BSC_chick_data %>%
  mutate(
    date_formatted = as.Date(date, format = "%d%m%Y")  # new column with proper Date format
  ) %>%
  group_by(nest_ID) %>%
...
# Step 3: Join funday count back to every row
BSC_chick_data_clean <- BSC_chick_data_clean %>%
  left_join(funday_summary_BSC, by = "nest_ID", relationship = "many-to-many")

```
```{r}
## FP
# Find brood start and end date
FP_chick_data_clean <- FP_chick_data %>%
  mutate(
    date_formatted = as.Date(date, format = "%d%m%Y")  # new column with proper Date format
  ) %>%
  group_by(nest_ID) %>%
...
# Step 3: Join funday count back to every row
FP_chick_data_clean <- FP_chick_data_clean %>%
  left_join(funday_summary_FP, by = "nest_ID", relationship = "many-to-many")

```
```{r}
## PI
# Find brood start and end date
PI_chick_data_clean <- PI_chick_data %>%
  mutate(
    date_formatted = as.Date(date, format = "%d%m%Y")  # new column with proper Date format
  ) %>%
  group_by(nest_ID) %>%
...
# Step 3: Join funday count back to every row
PI_chick_data_clean <- PI_chick_data_clean %>%
  left_join(funday_summary_PI, by = "nest_ID", relationship = "many-to-many")
...
Chick_mgmt_s_hatch_date_surv <- MP_chicks_Phi_run[[11]]$results$real ## I've added this one so not sure how to put into analysis below
