...
BC_chick_ch %>%
  count(mgmt_l) %>%
  arrange(mgmt_l) 
#to fix nests with management level NA
BC_chick_data %>% 
 filter(is.na(management_level)) %>% 
 distinct(nest_ID)
...
Chick_p_reals_MP <- MP_chicks_p_run[[11]]$results$real
...
ggplot() +
  geom_line(data = Chick_p_reals_MP, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
  geom_ribbon(data = Chick_p_reals_MP, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
...
Chick_p_reals_MP <- Chick_p_reals_MP[which(Chick_p_reals_MP$X1 == "p"),]

# clean dataframe
Chick_p_reals_MP <- Chick_p_reals_MP[,-c(1:11)]
Chick_p_reals_MP$Age <- "Chick"
row.names(Chick_p_reals_MP) <- NULL
```
...
MP_Chick_funday_mgmt_l_surv  <- MP_chicks_Phi_run[[14]]$results$real
