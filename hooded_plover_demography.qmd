...
MP_chicks_p_run$model.table %>% 
  ...
  fmt_number(columns = DeltaAICc, rows = 1:10, decimals = 2, use_seps = FALSE) %>% 
  fmt_number(columns = weight, rows = 1:10, decimals = 3, use_seps = FALSE) %>% 
...
MP_chicks_Phi_run$model.table %>%
  filter(DeltaAICc < 2 | row_number() == 13) %>%
  # replace text in model names
  mutate(
    model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
...
MP_Chick_funday_mgmt_s_surv$mgmt_l <- unlist(substr(MP_Chick_funday_mgmt_s_surv$X2, 10, 10))
...
MP_Chick_funday_surv$mgmt_l <- unlist(substr(MP_Chick_funday_surv$X2, 10, 10))

# plotting top model MP_Chick_funday_surv
...
BSC_chicks_Phi_run <- readRDS(file = "output/BSC_chicks_p_run.rds") 
