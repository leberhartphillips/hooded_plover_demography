                                       ...
                                       ifelse(rope_fence == 1, 3, 
                                              ...
                                              ifelse(sign_nest == 1, 2,
                                                     ...
                                                     ifelse((perm_fence == 1 & nest_hab == "dune") | sign_access == 1, 1,
                                                            ifelse(none == 1, 0, NA))))))%>%
    mutate(management_status = ifelse(management_level == 1, "N", management_status)) #changes level 1 management to management status N
  }
...
MP_nests_with_issues <- 
  ...
  mutate(issue6 = ifelse(is.na(last_checked), "last checked date missing; ", NA)) %>% 
  mutate(issue7 = ifelse(management_status %!in% c("Y", "N"), "Nest managed? is not Y or N; ", NA)) %>% 
  mutate(issue8 = ifelse(nest_hab %!in% c("beach", "dune", "foredune/face", "estuary/spit"), "Nest habitat is not Beach, Dune, Foredune/face, or Estuary/spit; ", NA)) %>% 
  mutate(issue9 = ifelse(is.na(management_level), "Management type is not sufficient for making levels; ", NA)) %>%
  mutate(found_and_alive_diff = last_alive2 - first_found2) %>% 
...
nest_data_BSC%>% summarise(total_nests = n())
...
nest_data_BC_check <-
  ...
  mutate(first_found2_trans = as.Date(paste("2020", first_found2_md, sep = "-"), format = "%Y-%m-%d") - 179,
         last_alive2_trans = as.Date(paste("2020", last_alive2_md, sep = "-"), format = "%Y-%m-%d") - 179,
         last_checked2_trans = as.Date(paste("2020", last_checked2_md, sep = "-"), format = "%Y-%m-%d") - 179) %>% 
  mutate(season = ifelse(as.character(season) == "201820", "201819", as.character(season)) %>% as.factor()) %>% 
  mutate(season_label = paste0("season ", str_sub(season, 1, 4), " to ", str_sub(season, 5, 6)))
```
...
# check if there are any data in which the last alive date is a) beyond the nocc, b) less than 1, or c) NA 
filter(nest_data_BC, LastPresent > nocc | LastPresent < 1 | is.na(LastPresent)) # should be nothing if correct
# check if there are any data in which the found date is a) beyond the nocc, or b) less than 1
filter(nest_data_BC, FirstFound > nocc | FirstFound < 1) # should be nothing if correct
# check if there are any data in which the found date is a) after the last alive or b) after the last checked date 
filter(nest_data_BC, FirstFound > LastPresent | FirstFound > LastChecked)  # should be nothing if correct

# extract nests with issues
BC_nests_with_issues <- 
  nest_data_BC %>% 
  mutate(issue1 = ifelse(nchar(first_found) != 8, "found date is not 8 characters; ", NA)) %>% 
  mutate(issue2 = ifelse(nchar(last_alive) != 8, "last seen alive date is not 8 characters; ", NA)) %>% 
  mutate(issue3 = ifelse(nchar(last_checked) != 8, "last checked date is not 8 characters; ", NA)) %>% 
...
# see a table of the types of issues (5 with incubation > 35 days and 1 with Found date is after Last Alive date)
BC_nests_with_issues %>% pull(issues) %>% table()
BC_nests_with_issues %>% select(issues, season, site)

# remove nests with issues
nest_data_BC <- 
  nest_data_BC %>% 
  filter(nest_ID %!in% BC_nests_with_issues$nest_ID)
```
### Phillip Island
...
# check if there are any data in which the last alive date is a) beyond the nocc, b) less than 1, or c) NA 
filter(nest_data_PI, LastPresent > nocc | LastPresent < 1 | is.na(LastPresent)) # should be nothing if correct
# check if there are any data in which the found date is a) beyond the nocc, or b) less than 1
filter(nest_data_PI, FirstFound > nocc | FirstFound < 1) # should be nothing if correct
# check if there are any data in which the found date is a) after the last alive or b) after the last checked date 
filter(nest_data_PI, FirstFound > LastPresent | FirstFound > LastChecked)  # should be nothing if correct

# extract nests with issues
PI_nests_with_issues <- 
  nest_data_PI %>% 
  mutate(issue1 = ifelse(nchar(first_found) != 8, "found date is not 8 characters; ", NA)) %>% 
  mutate(issue2 = ifelse(nchar(last_alive) != 8, "last seen alive date is not 8 characters; ", NA)) %>% 
  mutate(issue3 = ifelse(nchar(last_checked) != 8, "last checked date is not 8 characters; ", NA)) %>% 
...
# see a table of the types of issues (5 with incubation > 35 days and 1 with Found date is after Last Alive date)
PI_nests_with_issues %>% pull(issues) %>% table()
PI_nests_with_issues %>% select(issues, season, site)

# remove nests with issues
nest_data_PI <- 
  nest_data_PI %>% 
  filter(nest_ID %!in% PI_nests_with_issues$nest_ID)
```
...
ggplot() +
  geom_jitter(data = threat_data__, 
              aes(x = weekday, y = log(humans_ + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot() +
  geom_jitter(data = threat_data__, 
              aes(x = weekday, y = log(dogs_on_ + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot() +
  geom_jitter(data = threat_data__, 
              aes(x = weekday, y = log(dogs_off_ + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot() +
  geom_jitter(data = threat_data__, 
              aes(x = weekday, y = log(pred_birds_ + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot() +
  geom_jitter(data = threat_data__, 
              aes(x = weekday, y = log(vehicles_ + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot(data = threat_data__) +
  geom_jitter(aes(x = weekday, y = log(hum_pri + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot(data = threat_data__) +
  geom_jitter(aes(x = weekday, y = log(dog_pri + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot(data = threat_data__) +
  geom_jitter(aes(x = weekday, y = log(vehicle_pri + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
ggplot(data = threat_data__) +
  geom_jitter(aes(x = weekday, y = log(fox_pri + 1)),
    width = 0.1,
    height = 0,
    color = "grey70",
    alpha = 0.15,
...
```{r, echo=FALSE}
nest_data_BC_with_threat_data <- readRDS(file = "output/nest_data_BC_with_threat_data.rds")
```
...
```{r, echo=FALSE}
nest_data_PI_with_threat_data <- readRDS(file = "output/nest_data_PI_with_threat_data.rds")
```

remove outlier data and those with very infrequent threat surveys (cut off: visited at least once per week)
```{r}
# NOTE: for future analysis of threats, need to consider restricting dataset 
# to nests that have an appropriate number threat surveys (i.e., at least one 
# during the nest's life or at least visited once per week). Currently, these
# restrictions are ignored because of Jasmin's focus on the "fundays" effect, 
# which is not reliant on threat surveys.
...
#nest_data_with_threat_data_7d <-  #removed this as only look at fundays so threat surveys dont matter
 # nest_data_with_threat_data %>% 
...
```{r}
write.csv(nest_data_with_threat_data_7d_, "data/nest_data_with_threat_data_7d_31102025.csv")
```
...
```{r}
#Summary Table
nests_per_year <- nest_data_with_threat_data_7d_ %>%
  group_by(season, region) %>%
  summarise(n_nests = n(), .groups = "drop") %>%
  mutate(season = gsub("(^\\d{4})(\\d{2}$)", "\\1–\\2", season),
    season_order = as.numeric(substr(season, 1, 4))) %>%
  ...
  pivot_wider(
    ...
    values_from = n_nests,
    values_fill = 0) %>%
  rename_with(~ ifelse(.x %in% names(region_names_no_break), region_names_no_break[.x], .x))

numeric_cols <- names(nests_per_year)[names(nests_per_year) != "season"]

total_row <- nests_per_year %>%
  summarise(across(all_of(numeric_cols), sum)) %>%
  mutate(season = "Total") %>%
  select(names(nests_per_year)) # ensure column order matches

nests_per_year <- bind_rows(nests_per_year, total_row)

nests_per_year <- nests_per_year %>%
  mutate(across(all_of(numeric_cols), ~ ifelse(. == 0, "-", as.character(.))))

nests_per_year %>%
  gt() %>%
  cols_label(season = html("<i>Season</i>")) %>%
  cols_align(align = "left", columns = "season") %>%
  text_transform(
    locations = cells_body(columns = "season"),
    fn = function(x) {
      paste0('<span style="white-space: nowrap;">', x, '</span>')
    }) %>%
...
```{r}
nest_data_with_threat_data_7d_ <- 
  read.csv("data/nest_data_with_threat_data_7d_31102025.csv") %>% 
...
saveRDS(nest_survival_FP, file = "output/nest_survival_FP_threats&status_31102025.rds")
```
...
saveRDS(nest_survival_MP, file = "output/nest_survival_MP_threats&status_31102025.rds")
```
...
saveRDS(nest_survival_BSC, file = "output/nest_survival_BSC_threats&status_31102025.rds")
```
...
saveRDS(nest_survival_BC, file = "output/nest_survival_BC_threats&status_31102025.rds")
```
...
saveRDS(nest_survival_PI, file = "output/nest_survival_PI_threats&status_31102025.rds")
```

# Results
### model selection results of threats
Note that Fundays is not included in these tables because it is inherently a different type of variable that the variables quantified through the threat surveys. But focusing on the actual threats we can pinpoint which specific threat is relevant for nest survival in a given population. Fundays by far explains the most variation nest survival for all populations, yet we can't interpret the role of, for example, gulls on nest survival when looking at fundays alone.
```{r}
nest_survival_FP <- readRDS(file = "output/nest_survival_FP_threats&status_31102025.rds")
nest_survival_run_FP <- nest_survival_FP$model_selection

FP_threats_AIC_nest_survival_table <- 
nest_survival_run_FP$model.table %>% 
  dplyr::select(model, npar, DeltaAICc, weight) %>%
  mutate(
    model = model %>%
      str_remove_all("^S\\(~|\\)$") %>%    # remove S(~ at start and ) at end
      str_replace_all("pbd_m", "(max. no. pred. birds)") %>%
      str_replace_all("dof_m", "(max. no. unleashed dogs)") %>%
      str_replace_all("dog_m", "(max. no. dogs)") %>%
      str_replace_all("gul_m", "(max. no. gulls)") %>%
      str_replace_all("hum_m", "(max. no. humans)") %>%
      str_replace_all("veh_m", "(max. no. vehicles)") %>%
      str_replace_all("status", "(mgmt Y/N)") %>%
      str_replace_all("1", "intercept only")
  ) %>% 
  gt() %>% 
  ...
  tab_options(row_group.font.weight = "bold",
              ...
              table.width = pct(50)) %>%
  tab_header(
    title = "Hooded Plover daily nest survival",
    subtitle = "Relative importance of specific threats and management action"
  )

gtsave(data = FP_threats_AIC_nest_survival_table, 
       filename = "FP_threats_AIC_nest_survival_table.png", path = "results/")

nest_survival_MP <- readRDS(file = "output/nest_survival_MP_threats&status_31102025.rds")
nest_survival_run_MP <- nest_survival_MP$model_selection

MP_threats_AIC_nest_survival_table <- 
...
gtsave(data = MP_threats_AIC_nest_survival_table, 
       filename = "MP_threats_AIC_nest_survival_table.png", path = "results/")

nest_survival_BSC <- readRDS(file = "output/nest_survival_BSC_threats&status_31102025.rds")
nest_survival_run_BSC <- nest_survival_BSC$model_selection

BSC_threats_AIC_nest_survival_table <- 
...
gtsave(data = BSC_threats_AIC_nest_survival_table, 
       filename = "BSC_threats_AIC_nest_survival_table.png", path = "results/")

nest_survival_BC <- readRDS(file = "output/nest_survival_BC_threats&status_31102025.rds")
nest_survival_run_BC <- nest_survival_BC$model_selection

BC_threats_AIC_nest_survival_table <- 
  ...
  cols_label(model = html("<i>Bass Coast</i>"),
             npar = "K",
             DeltaAICc = "DeltaAICc",
             weight = "weight")

gtsave(data = BC_threats_AIC_nest_survival_table, 
       filename = "BC_threats_AIC_nest_survival_table.png", path = "results/")

nest_survival_PI <- readRDS(file = "output/nest_survival_PI_threats&status_31102025.rds")
nest_survival_run_PI <- nest_survival_PI$model_selection

PI_threats_AIC_nest_survival_table <- 
...
gtsave(data = PI_threats_AIC_nest_survival_table, 
       filename = "PI_threats_AIC_nest_survival_table.png", path = "results/")
```

## Effect of weekends and holidays on nest survival
another way to assess the impact of humans is to consider the number of "Fundays" a nest is exposed to (i.e., the number of intensive-human-use days, such as weekends and holidays). 
```{r}
S.fundays <- 
...
FP_Beta_summary_fundays <- S.fundays$results$beta
...
MP_Beta_summary_fundays <- S.fundays$results$beta
...
BC_Beta_summary_fundays <- S.fundays$results$beta
...
BSC_Beta_summary_fundays <- S.fundays$results$beta
...
PI_Beta_summary_fundays <- S.fundays$results$beta

PI_Beta_summary_fundays
FP_Beta_summary_fundays
MP_Beta_summary_fundays
BSC_Beta_summary_fundays
BC_Beta_summary_fundays
...
fundays_DSR_plot <- 
  ...
  scale_x_continuous(
    breaks = c(0, 5, 10, 15, 20),  # keep your markers
    limits = c(0, 25),                 # full x-axis range
    expand = c(0, 0)) +
  luke_theme +
  ...
  xlab("Number of fundays (weekend days and holidays) exposed to") + 
  ylab("Estimated daily survival rate (± 95% CI)") +
  ylim(c(0, 1)) +
  facet_grid(. ~ region, labeller = as_labeller(region_names_break))

fundays_DSR_plot

ggsave(plot = fundays_DSR_plot, 
       filename = "fundays_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```

## Effect of management on mitigating the impact of weekends and holidays on nest survival
Here we evaluate how effective management action (i.e., binary: managed or not) is on dampening the impact of fundays on daily nest survival.
```{r}
S.fundays_status <- 
  mark(data = RMark_data_FP$nest_data.processed, 
       ddl = RMark_data_FP$nest_fate.ddl, 
...
FP_Beta_summary_status <- S.fundays_status$results$beta
...
MP_Beta_summary_status <- S.fundays_status$results$beta
...
BC_Beta_summary_status <- S.fundays_status$results$beta
...
BSC_Beta_summary_status <- S.fundays_status$results$beta
...
PI_Beta_summary_status <- S.fundays_status$results$beta

PI_Beta_summary_status
FP_Beta_summary_status
MP_Beta_summary_status
BSC_Beta_summary_status
BC_Beta_summary_status
...
fundays_mgmt_status_DSR_plot <- 
  ggplot(pred.fundays_status, 
       aes(x = covdata, y = estimate)) +
  geom_line(size = 1, aes(color = status)) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl, fill = status), alpha = 0.5) +
  ...
  scale_x_continuous(
    ...
    expand = c(0, 0)) +
  luke_theme +
  theme(legend.position = c(0.19, 0.1),
...
ggsave(plot = fundays_mgmt_status_DSR_plot, 
       filename = "fundays_mgmt_status_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```
...
mgmt_level_plot <- 
  ...
  geom_line(data = nest_survival_reals_levels, 
            ...
            position = position_dodge(width = 0.5), alpha = 0.5, size = 1) +
...
ggsave(plot = mgmt_level_plot, 
       filename = "mgmt_level_plot.png", 
       path = "results/", height = 4, width = 4)
```
...
## results table
nest_survival_reals_status_table <- nest_survival_reals_status %>%
  mutate(
    region = dplyr::recode(region, !!!region_names_no_break),
    status = dplyr::recode(status, "N" = "Unmanaged", "Y" = "Managed"),
     Hatching_rate = (estimate ^ 28),
    lcl_hatch_rate = (lcl ^ 28),
    ucl_hatch_rate = (ucl ^ 28)) %>% 
  mutate(HR_cl = paste0(round(Hatching_rate, 2), " [", round(lcl_hatch_rate, 2), " - ", round(ucl_hatch_rate, 2), "]"),
         DSR_cl = paste0(round(estimate, 2), " [", round(lcl, 2), " - ", round(ucl, 2), "]"))

mgmt_status_summary_table <- 
  nest_survival_reals_status_table %>%
  select(region, status, DSR_cl, HR_cl) %>% 
  arrange(region, status) %>%
  gt(groupname_col = "region") %>%
  cols_label(
    status = "",
    DSR_cl = "Daily nest survival [95% CI]",
    HR_cl = "Hatching success [95% CI]") %>%
  fmt_number(
    columns = c(DSR_cl, HR_cl),
    decimals = 2,
    use_seps = FALSE) %>%
  # fmt_number(
  #   columns = vars(Hatching_rate),
...
gtsave(data = mgmt_status_summary_table, 
       filename = "mgmt_status_summary_table.png", path = "results/")

# #hatch rate graph
# unique_regions <- unique(nest_hatch_rate$region)
# region_cols <- setNames(brewer.pal(length(unique_regions), "Dark2"), unique_regions)
# 
# nest_survival_reals_status_table <- nest_survival_reals_status_table %>%
#   mutate(fill_col = ifelse(
#     status == "Managed",
#     alpha(region_cols[region], 0.5),  # lighter Managed
#     region_cols[region]                # solid Unmanaged
#   ))
# # hatching rate bar chart
# ggplot(nest_survival_reals_status_table,
#        aes(x = region, y = Hatching_rate, fill = region, alpha = status)) +
#   geom_col(position = position_dodge(width = 0.8), color = NA, width = 0.7) +
#   geom_errorbar(aes(ymin = lcl_hatch_rate, ymax = ucl_hatch_rate),
#                 width = 0.2, position = position_dodge(width = 0.8)) +
#   scale_fill_manual(values = region_cols, guide = FALSE) +
#   scale_alpha_manual(name = NULL,
#     values = c("Unmanaged" = 1, "Managed" = 0.5),
#     labels = c("Unmanaged (darker)", "Managed (lighter)")) +
#   guides(alpha = guide_legend(override.aes = list(fill = "grey50", color = NA))) + 
#   coord_cartesian(ylim = c(0, 50)) +
#   labs(x = "Region", y = "Hatching rate (%)") +
...
daily_nest_survival_mgmt_status_plot <- 
  ggplot() +
...
hatching_success_mgmt_status_plot <- 
  ggplot() +
  geom_line(data = nest_survival_reals_status_table, 
            aes(x = status, y = Hatching_rate, color = region, group = region),
            position = position_dodge(width = 0.5), alpha = 0.5, size = 1) +
  geom_errorbar(data = nest_survival_reals_status_table, 
                aes(ymin = lcl_hatch_rate, ymax = ucl_hatch_rate, 
                    x = status, 
                    y = estimate, group = region), position = position_dodge(width = 0.5),
                alpha = 0.5, color = "black", width = 0.3, lwd = 0.5) +
  geom_point(data = nest_survival_reals_status_table, 
             aes(x = status, y = Hatching_rate, fill = region),
             shape = 21, size = 4, position = position_dodge(width = 0.5)) +
  # scale_colour_brewer(palette = "Set1") +
  luke_theme +
  theme(legend.position = c(0.55, 0.5),
        ...
        legend.background = element_rect(fill = "transparent")) +
  ylab("Estimated hatching success (± 95% CI)") +
  ylim(c(0, 0.5)) +
  scale_colour_brewer(palette = "Dark2", 
                      name = "Region", 
                      labels = c("BSC" = "Bellarine-Surf Coast", 
...
combo_mgmt_status_nest_surv_plot <- 
  hatching_success_mgmt_status_plot / daily_nest_survival_mgmt_status_plot

combo_mgmt_status_nest_surv_plot

ggsave(plot = combo_mgmt_status_nest_surv_plot, 
       filename = "combo_mgmt_status_nest_surv_plot.png", 
       path = "results/", height = 8, width = 4)
```
...
ggsave(plot = habitat_nest_surv_plot, 
       filename = "habitat_nest_surv_plot.png", 
       path = "results/", height = 4, width = 4)
```
...
ggsave(plot = dogs_DSR_plot, 
       filename = "dogs_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```
...
ggsave(plot = dogs_unleashed_DSR_plot, 
       filename = "dogs_unleashed_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```
...
ggsave(plot = humans_DSR_plot, 
       filename = "humans_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```
...
ggsave(plot = pred_birds_DSR_plot, 
       filename = "pred_birds_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```
...
ggsave(plot = gulls_DSR_plot, 
       filename = "gulls_DSR_plot.png", 
       path = "results/", height = 4, width = 8)
```
...
region_labels <- nest_survival_reals_year %>%
  group_by(region) %>%
  summarise(
    y_top = 1.01
  ) %>%
  ungroup() %>%
  mutate(
    x = min(nest_survival_reals_year$season),  # left-aligned at panel start
    y = y_top + 0.04,                           # fixed offset above panel top
    label = region_names_no_break[as.character(region)]
  )

# 2️⃣ Clip error bars to panel limits
nest_survival_reals_year <- nest_survival_reals_year %>%
  mutate(
    lcl_clip = pmax(lcl, 0.7),
    ucl_clip = pmin(ucl, 1)
  )

annual_nest_surv_plot <- 
  ggplot() +
  ...
  geom_errorbar(data = nest_survival_reals_year, 
                aes(ymin = pmax(lcl,0.7), ymax = pmin(ucl,1), x = season, y = estimate, group = region),
  ...
  theme(legend.position = "none",
    ...
    plot.margin = margin(50, 10, 20, 10)) +
  xlab("Season") + 
  ylab("Estimated daily survival rate (± 95% CI)") +
...
ggsave(plot = annual_nest_surv_plot, 
       filename = "annual_nest_surv_plot.png", 
       path = "results/", height = 8, width = 8)
```
...
nest_survival_season_plot_FP <- 
  ...
  geom_ribbon(aes(x = date, ymin = lcl, ymax = ucl),
              fill = brewer.pal(8, "Dark2")[c(3)], 
              alpha = 0.5) +
  geom_line(aes(x = date, y = estimate), 
            color = brewer.pal(8, "Dark2")[c(3)],
            size = 1) +
  scale_x_date(date_labels = "%B", 
               expand = c(0.01, 0.01), 
...
nest_survival_season_plot_MP <- 
  ...
  geom_ribbon(aes(x = date, ymin = lcl, ymax = ucl),
              fill = brewer.pal(8, "Dark2")[c(4)], 
              alpha = 0.5) +
  geom_line(aes(x = date, y = estimate), 
            color = brewer.pal(8, "Dark2")[c(4)],
            size = 1) +
  scale_x_date(date_labels = "%B", 
               expand = c(0.01, 0.01), 
               date_breaks = "1 months") +
...
hooded_plover_nest_plot_BC <- 
  ...
  plot_layout(widths = c(5), 
              heights = unit(c(1, 3), c('in', 'in'))) +
  plot_annotation(tag_levels = 'A', title = "Bass Coast")
...
nest_survival_season_plot_PI <- 
  ...
  geom_ribbon(aes(x = date, ymin = lcl, ymax = ucl),
              fill = brewer.pal(8, "Dark2")[c(5)], 
              alpha = 0.5) +
  geom_line(aes(x = date, y = estimate), 
            color = brewer.pal(8, "Dark2")[c(5)],
            size = 1) +
  scale_x_date(date_labels = "%B", 
               expand = c(0.01, 0.01), 
               date_breaks = "1 months") +
...
hooded_plover_nest_plot_FP
ggsave(plot = hooded_plover_nest_plot_FP, 
       filename = "seasonal_nest_surv_plot_FP.png", 
       path = "results/", height = 8, width = 8)

hooded_plover_nest_plot_MP
ggsave(plot = hooded_plover_nest_plot_MP, 
       filename = "seasonal_nest_surv_plot_MP.png", 
       path = "results/", height = 8, width = 8)

hooded_plover_nest_plot_BC
ggsave(plot = hooded_plover_nest_plot_BC, 
       filename = "seasonal_nest_surv_plot_BC.png", 
       path = "results/", height = 8, width = 8)

hooded_plover_nest_plot_BSC
ggsave(plot = hooded_plover_nest_plot_BSC, 
       filename = "seasonal_nest_surv_plot_BSC.png", 
       path = "results/", height = 8, width = 8)

hooded_plover_nest_plot_PI
ggsave(plot = hooded_plover_nest_plot_PI, 
       filename = "seasonal_nest_surv_plot_PI.png", 
       path = "results/", height = 8, width = 8)
```
...
## Find out number of nests with clutch comp dates and without
hatch_dates_BSC_MP_FP_BC_PI %>% 
  summarise(
    with_clutch_comp = sum(!is.na(clutch_comp)),
    without_clutch_comp = sum(is.na(clutch_comp))
  )


## Find out number of nests per management level that have clutch comp dates
hatch_dates_BSC_MP_FP_BC_PI %>% 
  filter(!is.na(clutch_comp)) %>%        # keep nests with clutch_comp
  group_by(management_level) %>%       # group by management level
  summarise(n_obs = n())        # count nests per level
```
...
MP_nest_ID <-
  suppressMessages(
    ...
    read_excel(paste0("data/", "MP Nesting Summary All Years_19062025.xlsx"),
               sheet = paste0("MP", " ", "2012", "_", str_sub("2013", 3, 4)),
               col_types = "text", na = "n/a")))%>% 
  dplyr::select(`Nest ID`, `Chick manged?`, `Chick management type`) %>%
  rename(nest_ID = `Nest ID`,
    management_status = `Chick manged?`,
    management_type = `Chick management type`)
  ...
  suppressMessages(
   ...
   rename(region = Region, 
    ...
    left_join(., hatch_dates_BSC_MP_FP_BC_PI %>% 
                dplyr::select(nest_ID, hatch_date), by = "nest_ID") %>% 
  left_join(., MP_nest_ID %>% 
                dplyr::select(nest_ID, management_type, management_status), by = "nest_ID", relationship = "many-to-many")
...
age_offsets <- c(
  ...
  "2/3 adult size, mottled grey" = -19,
...
hatch_dates_from_chick_data_MP <- MP_chick_data %>%
  ...
  dplyr::select(nest_ID, hatch_date2) %>%
  ungroup()

# merge data
MP_chick_data_clean_hatch_dates <- 
  MP_chick_data %>%
  left_join(., hatch_dates_from_chick_data_MP, by = "nest_ID") %>% 
...
MP_chick_data_clean_hatch_dates <- 
  MP_chick_data %>%
  left_join(., hatch_dates_from_chick_data_MP, by = "nest_ID") %>% 
  dplyr::select(nest_ID, hatch_date, hatch_date2, chick_age, date) %>% 
  distinct() %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date%>%
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>% 
...
MP_chick_data <-
  ...
  #mutate(management_type = str_replace_all(management_type, "shelters", "")) %>%
  mutate(management_type = str_replace_all(management_type, ",,", ",")) %>%
  mutate(temporary_exclusion_zone = ifelse(str_detect(management_type, "temporaryexclusionzone"), 1, 0)) %>%
  mutate(banners = ifelse(str_detect(management_type, "banners"), 1, 0)) %>%
  mutate(sign_access = ifelse(str_detect(management_type, "signaccess"), 1, 0)) %>%
  mutate(sign_nest = ifelse(str_detect(management_type, "signnest"), 1, 0)) %>%
  mutate(rope_fence = ifelse(str_detect(management_type, "ropefence"), 1, 0)) %>%
  mutate(perm_fence = ifelse(str_detect(management_type, "permanentfence"), 1, 0)) %>%
  mutate(shelters = ifelse(str_detect(management_type, "shelters"), 1, 0)) %>%
  mutate(wardens = ifelse(str_detect(management_type, "wardens"), 1, 0)) %>% 
  mutate(none = ifelse(str_detect(management_type, "none"), 1, 
                       ifelse(is.na(management_type) | management_type == "", 1, 0))) %>%
  mutate(other = ifelse(str_detect(management_type, "other"), 1, 0)) %>%
  mutate(management_level = ifelse(temporary_exclusion_zone == 1, 6,
                                   ifelse(wardens == 1, 5,
                                          ifelse((rope_fence == 1 | perm_fence == 1), 4,
                                                 ifelse(banners == 1, 3,
                                                        ifelse(sign_nest == 1, 2,
 ...
 # dplyr::select(-other, -sign_nest_no_sign_access, -fence_no_sign, 
              #  -wardens_no_sign, -wardens_no_fence, -just_wardens) %>%
  mutate(management_status = case_when(
    str_detect(management_type, regex("shelters", ignore_case = TRUE)) ~ "Y",  # shelters present
    management_level > 1 ~ "Y",                                               # strong management
    management_level %in% c(0, 1) ~ "N",                                      # minimal or none
 ...
 mutate(region = ifelse(str_detect(region, "Mornington"), "MP",
                                    ifelse(str_detect(region, "Fleurieu"), "FP",
                                              ifelse(str_detect(region, "Phillip"), "PI",
                                                           ifelse(str_detect(region, "Bass"), "BC",
...
MP_chick_data_issues %>% dplyr::select(-c(date, management_status, management_level)) %>% 
  distinct() %>% 
  pull(issues) %>% 
  table()
```
...
MP_chick_data_clean <-
  ...
  dplyr::select(region, season, site, nest_ID, hatch_date, date, 
...
```{r}
# ##MP
# # Jas and Mike attempt to calculate fundays # need to max fundays = 35
# # Find brood start and end date
# MP_chick_data_clean <- MP_chick_data_clean %>%
#   mutate(
#     date_formatted = as.Date(date, format = "%d%m%Y")  # new column with proper Date format
#   ) %>%
#   group_by(nest_ID) %>%
#   mutate(
#     last_checked = max(date_formatted, na.rm = TRUE)  # latest date per nest_ID
#   ) %>%
#   ungroup()
# #clean
# MP_chick_data_clean <- MP_chick_data_clean %>%
...
#     last_checked = as.Date(last_checked)
#   ) %>%
#   filter(!is.na(hatch_date), !is.na(last_checked), hatch_date <= last_checked)
# 
# 
# funday_summary_MP <- 
#   MP_chick_data_clean %>%
...
#   mutate(
#     weekend_days = ifelse(
#       is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked,
#       0L,
#       sum(weekdays(seq(hatch_date, last_checked, by = "day")) %in% c("Saturday", "Sunday"))
#     ),
#     holiday_days = ifelse(
#       is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked,
#       0L,
#       sum(sapply(
#         seq(hatch_date, last_checked, by = "day"),
#         function(d) {
#           is_weekend <- weekdays(d) %in% c("Saturday", "Sunday")
#           is_holiday <- any(
#             !is.na(holidays$start_date) &
#             !is.na(holidays$end_date) &
#             d >= holidays$start_date &
#             d <= holidays$end_date &
#             holidays$region == "MP"
#           )
#           as.integer(is_holiday & !is_weekend)
#         }
#       ), na.rm = TRUE)
#     ),
#     funday_total = pmin(weekend_days + holiday_days, 35L)
#   ) %>%
#   ungroup()
# 
# # Step 3: Join funday count back to every row
# MP_chick_data_clean <- MP_chick_data_clean %>%
#   left_join(
#     funday_summary_MP %>% dplyr::select(nest_ID, funday_total),
#     by = "nest_ID",
#     relationship = "many-to-many")
```

```{r}
# Step 1. Expand holidays into daily rows (as before)
holidays_expanded <- holidays %>%
  filter(!is.na(start_date) & !is.na(end_date)) %>%
  mutate(day = map2(start_date, end_date, seq, by = "day")) %>%  # rename here
  unnest(day) %>%
  select(season, region, day) %>%
  distinct()

# Step 2: Process chick data with special days
MP_chick_data_specialdays <- MP_chick_data_clean %>%
  select(region, season, nest_ID, brood_age, hatch_date, date, offspring_seen) %>%
  mutate(hatch_date = hatch_date + 1) %>% 
  arrange(nest_ID, date) %>%
  group_by(nest_ID) %>%
  mutate(start_date = if_else(row_number() == 1, hatch_date, lag(date))) %>%
  ungroup() %>%
  rowwise() %>%
  mutate(
    # Only create sequences if both start_date and date are non-missing and start_date <= date
    all_days = if (is.na(start_date) | is.na(date) | start_date > date) {
      list(as.Date(character(0)))
    } else {
      list(seq(start_date, date, by = "day"))
    }
  ) %>%
  ungroup() %>%
  unnest(all_days) %>%
  rename(day = all_days) %>%
  left_join(holidays_expanded, by = c("season", "region", "day")) %>%
  mutate(
    is_weekend = weekdays(day) %in% c("Saturday", "Sunday"),
    is_holiday = !is.na(season),  # after join, holiday rows have non-NA season
    special_day = is_weekend | is_holiday
  ) %>%
  group_by(region, season, nest_ID, date, hatch_date) %>%
  summarise(specialdays = sum(special_day), .groups = "drop") %>%
  arrange(nest_ID, date) %>%

  # integrate with original data and accumulate since last sighting
  left_join(
    MP_chick_data_clean %>%
      mutate(hatch_date = hatch_date + 1) %>%
      select(region, season, nest_ID, date, hatch_date, offspring_seen),
    by = c("region", "season", "nest_ID", "date", "hatch_date")
  ) %>%
  mutate(fledge_date = hatch_date + 35) %>% 
  filter(date <= fledge_date) %>% 
  arrange(nest_ID, date) %>%
  group_by(nest_ID) %>%
  mutate(event_group = cumsum(offspring_seen == 1)) %>%
  group_by(nest_ID, event_group) %>%
  mutate(
    specialdays_since_seen = cumsum(specialdays),
    # KEY FIX: if this row has offspring_seen == 1, set the 'since' counter to 0
    specialdays_since_seen = if_else(offspring_seen == 1, 0L, specialdays_since_seen)
  ) %>%
  ungroup()

# Step 4. Optional summary table: last specialdays_since_seen per nest
nest_specialday_summary <- MP_chick_data_specialdays %>%
  group_by(nest_ID) %>%
  summarise(funday_total = last(na.omit(specialdays_since_seen)))

# Step 3: Join funday count back to every row
MP_chick_data_clean <- MP_chick_data_clean %>%
      mutate(hatch_date = hatch_date + 1) %>%
  left_join(
    nest_specialday_summary,
    by = "nest_ID",
    relationship = "many-to-many")
```
...
MP_chick_data_clean_ %>% 
  dplyr::select(nest_ID, brood_age, offspring_seen, funday_total) %>% 
  as.data.frame() %>% 
...
MP_chick_data_clean_wide <- 
  ...
  pivot_wider(
    id_cols = c(nest_ID, management_level, management_status, season, region, hatch_date, funday_total),
...
age_cols <- names(MP_chick_data_clean_wide)[-c(1:7)] %>%           # drop nest_ID
...
MP_chick_data_clean_wide <- MP_chick_data_clean_wide %>%
  select(nest_ID, management_level, management_status, season, region, hatch_date, funday_total, all_of(age_cols))
...
MP_chick_ch <-
  ...
  mutate(management_level = as.factor(management_level)) %>% 
  mutate(region = as.factor(region)) %>% 
  ungroup() %>% 
  rename(mgmt_l = management_level,
         mgmt_s = management_status,
         date_j = hatch_date_j,
         funday = funday_total) %>% 
  filter(nest_ID != "junk") %>% 
  dplyr::select(-hatch_date, -nest_ID)
```
...
BC_nest_ID <-
  suppressMessages(
    ...
    read_excel(paste0("data/", "Bass Coast_nesting summaries.xlsx"),
               sheet = paste0("BC", " ", "2012", "_", str_sub("2013", 3, 4)),
               col_types = "text", na = "n/a")))%>% 
  select(`Nest ID_2`, `Chick manged?`, `Chick management type`) %>%
  rename(nest_ID = `Nest ID_2`,
    management_status = `Chick manged?`,
    management_type = `Chick management type`)
...
BC_chick_data <-
  suppressMessages(
    bind_rows(
    read_excel(paste0("data/", "Bass Coast_Chick data.xlsx"), 
               sheet = paste0("BC", " ", "2024", "_", str_sub("2025", 3, 4)), 
               col_types = "text", na = "n/a"),
      read_excel(paste0("data/", "Bass Coast_Chick data.xlsx"), 
               sheet = paste0("BC", " ", "2023", "_", str_sub("2024", 3, 4)), 
    ...
    dplyr::select(region, season, site, nest_ID, date, n_adults_seen, n_juvs_seen, 
                  n_chicks_seen, habitat, chick_age, lat, lon) %>%
    left_join(., hatch_dates_BSC_MP_FP_BC_PI %>% 
                select(nest_ID, hatch_date), by = "nest_ID") %>% 
  left_join(., BC_nest_ID %>% 
                select(nest_ID, management_type, management_status), by = "nest_ID", relationship = "many-to-many")
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data_BC <- BC_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
# merge data
BC_chick_data_clean_hatch_dates <- 
  BC_chick_data %>%
  left_join(., hatch_dates_from_chick_data_BC, by = "nest_ID") %>% 
  filter(hatch_date == hatch_date2 | 
           (is.na(hatch_date) & !is.na(hatch_date2)) | 
           (!is.na(hatch_date) & is.na(hatch_date2)))
...
BC_chick_data <-
  BC_chick_data %>% 
  filter(nest_ID %in% BC_chick_data_clean_hatch_dates$nest_ID) %>% 
  mutate(management_type = tolower(management_type)) %>%
  mutate(habitat = tolower(habitat)) %>%
  mutate(management_type = str_replace(management_type, "acess", "access")) %>%
  mutate(management_type = str_replace(management_type, "and", ",")) %>%
...
BC_chick_data_issues %>% select(-c(date, management_status, management_level)) %>% 
  distinct() %>% 
  pull(issues) %>% 
  table()
```
...
BC_chick_data_clean <-
  ...
  select(region, season, site, nest_ID, hatch_date, date, 
         n_adults_seen, n_juvs_seen, n_chicks_seen, lat, lon, 
         management_level, management_status) %>% 
  mutate(hatch_date = hatch_date + 2) %>% 
  group_by(nest_ID) %>% 
  arrange(date) %>% 
  ungroup() %>% 
  mutate(n_adults_seen = ifelse(is.na(n_adults_seen), 0, n_adults_seen),
...
# #Fundays
# # Find brood start and end date
# BC_chick_data_clean <- BC_chick_data_clean %>%
#   mutate(
#     date_formatted = as.Date(date, format = "%d%m%Y")  # new column with proper Date format
#   ) %>%
#   group_by(nest_ID) %>%
...
#     last_checked = as.Date(last_checked)
#   ) 
# # Join threat and brood data to find out weekends and weekdays
# funday_summary_BC <- BC_chick_data_clean %>%
#   rowwise() %>%  
#   mutate(
#     weekend_days = ifelse(
#       is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked,
...
# BC_chick_data_clean <- BC_chick_data_clean %>%
#   left_join(
#     funday_summary_BC %>% select(nest_ID, funday_total),
#     by = "nest_ID",
#     relationship = "many-to-many")
...
# Step 2: Process chick data with special days
BC_chick_data_specialdays <- BC_chick_data_clean %>%
  select(region, season, nest_ID, brood_age, hatch_date, date, offspring_seen) %>%
  mutate(hatch_date = hatch_date + 1) %>% 
  arrange(nest_ID, date) %>%
  group_by(nest_ID) %>%
...
# Step 4. Optional summary table: last specialdays_since_seen per nest
nest_specialday_summary <- BC_chick_data_specialdays %>%
  group_by(nest_ID) %>%
  summarise(funday_total = last(na.omit(specialdays_since_seen)))
...
BC_chick_data_clean_ %>% 
  select(nest_ID, brood_age, offspring_seen, funday_total) %>% 
  as.data.frame() %>% 
  arrange(nest_ID, brood_age)

library(tidyselect)

BC_chick_data_clean_wide <- 
  BC_chick_data_clean_ %>%
  mutate(offspring_seen = as.character(offspring_seen)) %>% 
  pivot_wider(
    id_cols = c(nest_ID, management_level, management_status, season, region, hatch_date, funday_total),
...
# Get the age columns in numeric order
age_cols <- names(BC_chick_data_clean_wide)[-c(1:6)] %>%           # drop nest_ID
  sub("age_", "", .) %>%                     # remove prefix
  as.numeric() %>%                            # convert to numeric
  sort() %>%                                  # sort numerically
  paste0("age_", .)                           # add prefix back
...
# Reorder the dataframe
BC_chick_data_clean_wide <- BC_chick_data_clean_wide %>%
  select(nest_ID, management_level, management_status, season, region, hatch_date, funday_total, all_of(age_cols))
...
BSC_chick_data <-
     bind_rows(
     read_excel(paste0("data/", "BSC Chick data 13062025.xlsx"), 
               sheet = paste0("BSC", " ", "2024", "_", str_sub("2025", 3, 4)), 
               col_types = "text", na = "n/a"),
      read_excel(paste0("data/", "BSC Chick data 13062025.xlsx"), 
               sheet = paste0("BSC", " ", "2023", "_", str_sub("2024", 3, 4)), 
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data_BSC <- BSC_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
# merge data
BSC_chick_data_clean_hatch_dates <- 
  BSC_chick_data %>%
  left_join(., hatch_dates_from_chick_data_BSC, by = "nest_ID") %>% 
  filter(hatch_date == hatch_date2 | 
           (is.na(hatch_date) & !is.na(hatch_date2)) | 
           (!is.na(hatch_date) & is.na(hatch_date2)))
...
#### wrangle management data ----
  BSC_chick_data <-
  BSC_chick_data %>% 
  filter(nest_ID %in% BSC_chick_data_clean_hatch_dates$nest_ID) %>% 
  mutate(management_type = tolower(management_type)) %>%
  mutate(habitat = tolower(habitat)) %>%
  mutate(management_type = str_replace(management_type, "acess", "access")) %>%
  mutate(management_type = str_replace(management_type, "and", ",")) %>%
...
BSC_chick_data_issues <- 
  ...
  mutate(issues = ifelse(
    is.na(issue1) & is.na(issue2) & is.na(issue3) & is.na(issue4) &
      is.na(issue5) & is.na(issue6) & is.na(issue7),
    "usable",
    paste0(issue1, issue2, issue3, issue4, issue5, issue6, issue7)
  )) %>% 
  mutate(issues = str_remove_all(issues, "NA")) %>% 
  dplyr::select(-issue1, -issue2, -issue3, -issue4, -issue5, -issue6, -issue7) %>%
  arrange(issues)
...
BSC_chick_data_issues %>% select(-c(date, management_status, management_level)) %>% 
  distinct() %>% 
  pull(issues) %>% 
  table()
```
...
BSC_chick_data_clean <-
  ...
  mutate(n_adults_seen = ifelse(is.na(n_adults_seen), 0, n_adults_seen),
         n_juvs_seen = ifelse(is.na(n_juvs_seen), 0, n_juvs_seen),
         n_chicks_seen = ifelse(is.na(as.numeric(n_chicks_seen)), 0, n_chicks_seen)) %>% 
  mutate(n_offspring_seen = as.numeric(n_juvs_seen) + as.numeric(n_chicks_seen)) %>% 
  mutate(offspring_seen = ifelse(n_offspring_seen > 0, 1, 0)) %>% 
  mutate(brood_age = (date - hatch_date) %>% as.numeric()) %>% 
  group_by(nest_ID)# %>% 
...
# ## Fundays
# # Find brood start and end date
# BSC_chick_data_clean <- BSC_chick_data_clean %>%
#   mutate(date_formatted = as.Date(date, format = "%d%m%Y") ) %>%
#   group_by(nest_ID) %>%
#   mutate(last_checked = max(date_formatted, na.rm = TRUE)) %>%
#   ungroup()
# #clean
# BSC_chick_data_clean <- BSC_chick_data_clean %>%
#   mutate(
...
# 
# # Join threat and brood data to find out weekends and weekdays
# funday_summary_BSC <- BSC_chick_data_clean %>%
#   rowwise() %>%  
#   mutate(
#     weekend_days = ifelse(
#       is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked,
...
# BSC_chick_data_clean <- BSC_chick_data_clean %>%
#   left_join(
#     funday_summary_BSC %>% select(nest_ID, funday_total),
#     by = "nest_ID",
#     relationship = "many-to-many")
...
# Step 2: Process chick data with special days
BSC_chick_data_specialdays <- BSC_chick_data_clean %>%
  select(region, season, nest_ID, brood_age, hatch_date, date, offspring_seen) %>%
  mutate(hatch_date = hatch_date + 1) %>% 
  arrange(nest_ID, date) %>%
  group_by(nest_ID) %>%
...
# Step 4. Optional summary table: last specialdays_since_seen per nest
nest_specialday_summary <- BSC_chick_data_specialdays %>%
  group_by(nest_ID) %>%
  summarise(funday_total = last(na.omit(specialdays_since_seen)))
...
# Get the age columns in numeric order
age_cols <- names(BSC_chick_data_clean_wide)[-c(1:6)] %>%           # drop nest_ID
  sub("age_", "", .) %>%                     # remove prefix
  as.numeric() %>%                            # convert to numeric
  sort() %>%                                  # sort numerically
  paste0("age_", .)                           # add prefix back
...
# Reorder the dataframe
BSC_chick_data_clean_wide <- BSC_chick_data_clean_wide %>%
  select(nest_ID, management_level, management_status, season, region, hatch_date, funday_total, all_of(age_cols))
...
FP_chick_data <-
  suppressMessages(
    bind_rows(
    read_excel(paste0("data/", "FP Chick data 18062025.xlsx"), 
               sheet = paste0("FP", " ", "2023", "_", str_sub("2024", 3, 4)), 
               col_types = "text", na = "n/a"),
    read_excel(paste0("data/", "FP Chick data 18062025.xlsx"),
               sheet = paste0("FP", " ", "2022", "_", str_sub("2023", 3, 4)),
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data_FP <- FP_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
# merge data
FP_chick_data_clean_hatch_dates <- 
  FP_chick_data %>%
  left_join(., hatch_dates_from_chick_data_FP, by = "nest_ID") %>% 
  filter(hatch_date == hatch_date2 | 
           (is.na(hatch_date) & !is.na(hatch_date2)) | 
           (!is.na(hatch_date) & is.na(hatch_date2)))
...
FP_chick_data <-
  FP_chick_data %>% 
  filter(nest_ID %in% FP_chick_data_clean_hatch_dates$nest_ID) %>% 
  mutate(management_type = tolower(management_type)) %>%
  mutate(habitat = tolower(habitat)) %>%
  mutate(management_type = str_replace(management_type, "acess", "access")) %>%
  mutate(management_type = str_replace(management_type, "and", ",")) %>%
...
FP_chick_data_issues %>% select(-c(date, management_status, management_level)) %>% 
  distinct() %>% 
  pull(issues) %>% 
  table()
```
...
# ## Fundays
# # Find brood start and end date
# FP_chick_data_clean <- FP_chick_data_clean %>%
#   mutate(date_formatted = as.Date(date, format = "%d%m%Y")) %>%
#   group_by(nest_ID) %>%
#   mutate(last_checked = max(date_formatted, na.rm = TRUE)) %>%
#   ungroup()
# #clean
# FP_chick_data_clean <- FP_chick_data_clean %>%
#   mutate(hatch_date = as.Date(hatch_date),
#     last_checked = as.Date(last_checked)) %>%
#   filter(!is.na(hatch_date), !is.na(last_checked), hatch_date <= last_checked)
# 
# # Join threat and brood data to find out weekends and weekdays
# funday_summary_FP <- FP_chick_data_clean %>%
#   rowwise() %>%  # optional, can be removed in vectorized version
#   mutate(
#     weekend_days = ifelse(
#       is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked,
#       0L,
...
# # Step 3: Join funday count back to every row
# FP_chick_data_clean <- FP_chick_data_clean %>%
#   left_join(funday_summary_FP %>% select(nest_ID, funday_total),
#     by = "nest_ID",
#     relationship = "many-to-many")
...
# Step 2: Process chick data with special days
FP_chick_data_specialdays <- FP_chick_data_clean %>%
  select(region, season, nest_ID, brood_age, hatch_date, date, offspring_seen) %>%
  mutate(hatch_date = hatch_date + 1) %>% 
  arrange(nest_ID, date) %>%
  group_by(nest_ID) %>%
...
# Step 4. Optional summary table: last specialdays_since_seen per nest
nest_specialday_summary <- FP_chick_data_specialdays %>%
  group_by(nest_ID) %>%
  summarise(funday_total = last(na.omit(specialdays_since_seen)))
...
# Get the age columns in numeric order
age_cols <- names(FP_chick_data_clean_wide)[-c(1:6)] %>%           # drop nest_ID
  sub("age_", "", .) %>%                     # remove prefix
  as.numeric() %>%                            # convert to numeric
  sort() %>%                                  # sort numerically
  paste0("age_", .)                           # add prefix back
...
# Reorder the dataframe
FP_chick_data_clean_wide <- FP_chick_data_clean_wide %>%
  select(nest_ID, management_level, management_status, season, region, hatch_date, funday_total, all_of(age_cols))
...
PI_chick_data <-
  suppressMessages(
    bind_rows(
     read_excel(paste0("data/", "PI Chick data.xlsx"), 
               sheet = paste0("PI", " ", "2024", "_", str_sub("2025", 3, 4)), 
               col_types = "text", na = "n/a"),
      read_excel(paste0("data/", "PI Chick data.xlsx"), 
               sheet = paste0("PI", " ", "2023", "_", str_sub("2024", 3, 4)), 
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data_PI <- PI_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
# merge data
PI_chick_data_clean_hatch_dates <- 
  PI_chick_data %>%
  left_join(., hatch_dates_from_chick_data_PI, by = "nest_ID") %>% 
  filter(hatch_date == hatch_date2 | 
           (is.na(hatch_date) & !is.na(hatch_date2)) | 
           (!is.na(hatch_date) & is.na(hatch_date2)))
...
#### wrangle management data ----
 PI_chick_data <-
  PI_chick_data %>% 
  filter(nest_ID %in% PI_chick_data_clean_hatch_dates$nest_ID) %>% 
  mutate(management_type = tolower(management_type)) %>%
  mutate(habitat = tolower(habitat)) %>%
  mutate(management_type = str_replace(management_type, "acess", "access")) %>%
  mutate(management_type = str_replace(management_type, "and", ",")) %>%
...
PI_chick_data_issues %>% select(-c(date, management_status, management_level)) %>% 
  distinct() %>% 
  pull(issues) %>% 
  table()
```
...
PI_chick_data_clean <-
  PI_chick_data_issues %>% 
  filter(issues == "usable") %>%
  filter(!is.na(management_status)) %>%
  # mutate(hatch_date = ifelse(!is.na(hatch_date) & is.na(hatch_date2), hatch_date, hatch_date2) %>% as.Date()) %>% 
  mutate(date = dmy(date)) %>% 
  select(region, season, site, nest_ID, hatch_date, date, 
         n_adults_seen, n_juvs_seen, n_chicks_seen, lat, lon, 
...
# ## Fundays
# # Find brood start and end date
# PI_chick_data_clean <- PI_chick_data_clean %>%
#   mutate(date_formatted = as.Date(date, format = "%d%m%Y")) %>%
#   group_by(nest_ID) %>%
#   mutate(last_checked = max(date_formatted, na.rm = TRUE)) %>%
#   ungroup()
...
# 
# # Join threat and brood data to find out weekends and weekdays
# funday_summary_PI <- PI_chick_data_clean %>%
#  rowwise() %>%
#   mutate(
#     weekend_days = if (is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked) {
#       0L
#     } else {
#       all_days <- seq(hatch_date, last_checked, by = "day")
#       sum(weekdays(all_days) %in% c("Saturday", "Sunday"), na.rm = TRUE)
#     },
#     holiday_days = if (is.na(hatch_date) | is.na(last_checked) | hatch_date > last_checked) {
#       0L
#     } else {
#       all_days <- seq(hatch_date, last_checked, by = "day")
#       sum(sapply(all_days, function(d) {
#         is_weekend <- weekdays(d) %in% c("Saturday", "Sunday")
#         is_holiday <- any(
#           !is.na(holidays$start_date) &
#           !is.na(holidays$end_date) &
...
#           holidays$region == "PI"
#         )
#         as.integer(is_holiday & !is_weekend)  # only count holidays that are not weekends
#       }), na.rm = TRUE) },
#   
#   # Total fundays = weekends + holidays, capped at 35
#     funday_total = pmin(weekend_days + holiday_days, 35L)
#   ) %>%
#   ungroup()
# 
# # Step 3: Join funday count back to every row
# PI_chick_data_clean <- PI_chick_data_clean %>%
#   left_join(funday_summary_PI %>% select(nest_ID, funday_total),
#     by = "nest_ID",
#     relationship = "many-to-many")
...
# Step 2: Process chick data with special days
PI_chick_data_specialdays <- PI_chick_data_clean %>%
  select(region, season, nest_ID, brood_age, hatch_date, date, offspring_seen) %>%
  mutate(hatch_date = hatch_date + 1) %>% 
  arrange(nest_ID, date) %>%
  group_by(nest_ID) %>%
...
# Step 4. Optional summary table: last specialdays_since_seen per nest
nest_specialday_summary <- PI_chick_data_specialdays %>%
  group_by(nest_ID) %>%
  summarise(funday_total = last(na.omit(specialdays_since_seen)))
...
# Get the age columns in numeric order
age_cols <- names(PI_chick_data_clean_wide)[-c(1:6)] %>%           # drop nest_ID
  sub("age_", "", .) %>%                     # remove prefix
  as.numeric() %>%                            # convert to numeric
  sort() %>%                                  # sort numerically
  paste0("age_", .)                           # add prefix back
...
# Reorder the dataframe
PI_chick_data_clean_wide <- PI_chick_data_clean_wide %>%
  select(nest_ID, management_level, management_status, season, region, hatch_date, funday_total, all_of(age_cols))
...
###Summary statistics
```{r}
# Combine all chick_ch datasets
all_chick_ch <- bind_rows(
  BC_chick_ch,
  BSC_chick_ch,
  FP_chick_ch,
  MP_chick_ch,
...
# Number of broods per management status
all_chick_ch %>% count(mgmt_s)

# Number of broods per management level
all_chick_ch %>%
  mutate(mgmt_l = as.numeric(as.character(mgmt_l))) %>%  # convert factor -> character -> numeric
  count(mgmt_l) %>%
  arrange(mgmt_l)
...
## MP
### Mark-recapture modelling (with all management levels)
```{r}
...
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(MP_chicks.proc$nocc[1]-1),
                         funday = MP_chick_ch$funday[1:MP_chicks.proc$nocc[1]])
...
MP_chicks.ddl$Phi <- merge_design.covariates(MP_chicks.ddl$Phi, 
                                             quad_time, bygroup = FALSE, bytime = TRUE)
MP_chicks.ddl$Phi <- merge_design.covariates(MP_chicks.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```

### Detection probability modelling (with all management levels)
```{r}
...
MP_chicks_p_run # best structures under 2 DeltaAICc: p(~season + Quadratic), p(~season * Quadratic), p(~Quadratic)

saveRDS(MP_chicks_p_run, "output/chick_surv/MP/MP_chicks_p_run.rds")

#Create AIC model selection table

MP_chicks_p_run <- readRDS(file = "output/MP_chicks_p_run.rds") 

MP_chicks_p_run$model.table %>% 
  mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "time",
      "season" = "year"))) %>%
  dplyr::select(model, npar, DeltaAICc, weight) %>% gt() %>% 
  cols_label(model = html("<i>Mornington Peninsula</i>"), 
             npar = "K", DeltaAICc = "DeltaAICc", weight = "weight") %>% 
  fmt_number(columns = DeltaAICc, decimals = 2, use_seps = FALSE) %>% 
  fmt_number(columns = weight, decimals = 3, use_seps = FALSE) %>% 
  cols_align(align = "left", columns = vars(model)) %>% 
  tab_style(style = cell_text(weight = "bold"),
    locations = cells_body(
      columns = vars(model),
      rows = model == "Phi(~1)p(~1)")) %>% 
   text_transform(
    locations = cells_body(columns = vars(model)),
    fn = function(x) gsub("\\)", ") ", x) ) %>%
  tab_options(row_group.font.weight = "bold", 
              row_group.background.color = brewer.pal(9,"Greys")[3], 
              table.font.size = 12, data_row.padding = 3, 
              row_group.padding = 4, summary_row.padding = 2, 
              column_labels.font.size = 14, row_group.font.size = 12, 
              table.width = pct(80))

#Create AIC model selection table with delta AIC < 2
MP_chicks_p_run$model.table %>%
   filter(DeltaAICc < 2 | model == "Phi(~1)p(~1)") %>%
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
       "Time" = "time",
  ...
  fmt_number(columns = weight, decimals = 3, use_seps = FALSE) %>%
  cols_align(align = "left", columns = vars(model)) %>%
   text_transform(locations = cells_body(columns = vars(model)),
    fn = function(x) gsub("\\)", ") ", x) ) %>%
  tab_options(row_group.font.weight = "bold",
    row_group.background.color = brewer.pal(9, "Greys")[3],
    table.font.size = 12,
...
### Mark-recapture modelling (with only management levels 4 and 5)
```{r}
MP_chick_45_ch <- 
  MP_chick_ch %>% filter(mgmt_l %in% c("4", "5"))

# Create processed RMARK data format as CJS with "Year" as group
MP_chicks_45.proc <- process.data(MP_chick_45_ch, 
                                 model = "CJS",
                                 groups = c("season", "region", 
                                            "mgmt_l", "mgmt_s"))

# Create the design data
MP_chicks_45.ddl <- make.design.data(MP_chicks_45.proc)

# create a quadratic time variable
time <- c(0:(MP_chicks_45.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(MP_chicks_45.proc$nocc[1]-1),
                         funday = MP_chick_45_ch$funday[1:MP_chicks_45.proc$nocc[1]])
MP_chicks_45.ddl$p <- merge_design.covariates(MP_chicks_45.ddl$Phi, 
                                           quad_time, bygroup = FALSE, bytime = TRUE)
MP_chicks_45.ddl$Phi <- merge_design.covariates(MP_chicks_45.ddl$Phi, 
                                             quad_time, bygroup = FALSE, bytime = TRUE)
MP_chicks_45.ddl$Phi <- merge_design.covariates(MP_chicks_45.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```

### Detection probability modelling (with only management levels 4 and 5)
```{r}
# First assess the best model structure of encounter probability (p) while
# keeping survival (Phi) constant
MP_chicks_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = MP_chicks_45.proc, 
                             ddl = MP_chicks_45.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
...
# Run model selection to determine best structure of p
MP_chicks_45_p_run <- MP_chicks_p()

# inspect p AIC table to find top model
MP_chicks_45_p_run # best structures under 2 DeltaAICc: p(~season + Quadratic), p(~season * Quadratic), p(~Quadratic)

saveRDS(MP_chicks_45_p_run, "output/MP_chicks_p_run_45.rds")
...
Chick_p_reals_MP <- MP_chicks_p_run[[4]]$results$real

# wrangle dataframe to get annual estimates (grouped by "Year")
Groups <- data.frame(str_split_fixed(rownames(Chick_p_reals_MP), " ", n = 5))
Chick_p_reals_MP <- cbind(Groups, Chick_p_reals_MP)
Chick_p_reals_MP$season <- unlist(substr(Chick_p_reals_MP$X2, 2, 7))
Chick_p_reals_MP$age <- as.numeric(sub("^a", "", Chick_p_reals_MP$X4))

# plot of the quadratic age dependent relationship with detection probability
ggplot() +
  geom_line(data = Chick_p_reals_MP %>% filter(X1 == "p"), aes(x = age, y = estimate)) +
  geom_ribbon(data = Chick_p_reals_MP %>% filter(X1 == "p"), aes(x = age, ymin = lcl, ymax = ucl), alpha = 0.2) +
  ylab("daily detection probability of a brood") +
  xlab("brood age (days)")
```
### Survival modelling (Phi)
```{r}
# Find best structure of Phi while using p(~season * Quadratic)
MP_chicks_Phi <- function() 
{
  ...
  # Phi varies with number of fun days
  Phi.funday <-
    list(formula = ~ funday)
  
  # Additive effect of funday and management level
  Phi.funday.mgmt_l <-
    list(formula = ~ funday + mgmt_l)

  # Additive effect of funday and management status
  Phi.funday.mgmt_s <-
    list(formula = ~ funday + mgmt_s)
  
   # Interaction between funday and management level
  Phi.funday.x.mgmt_l <-
    list(formula = ~ funday * mgmt_l)

   # Interaction between funday and management status
  Phi.funday.x.mgmt_s <-
    list(formula = ~ funday * mgmt_s)
  
  # # best structures of p determined from previous analysis
  # p.quadage <-
  #   list(formula = ~Quadratic)
...
MP_chicks_45_Phi_run <- MP_chicks_Phi()

# inspect Phi AIC table to find top model
MP_chicks_45_Phi_run # best structure: Phi(~season * Quadratic)

saveRDS(MP_chicks_45_Phi_run, "output/MP_chicks_45_Phi_run.rds")

#Create AIC model selection table

MP_chicks_Phi_run <- readRDS(file = "output/MP_chicks_Phi_run.rds") 

MP_chicks_Phi_run$model.table %>%
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
       "date_j" = "hatch date",
       "Time" = "time",
      "season" = "year"))) %>%
  dplyr::select(model, npar, DeltaAICc, weight) %>% gt() %>% 
  ...
  cols_align(align = "left", columns = vars(model)) %>% 
   tab_style(style = cell_text(weight = "bold"),
    locations = cells_body(
      columns = vars(model),
      rows = model == "Phi(~1)p(~quadratic age)")) %>% 
   text_transform(locations = cells_body(columns = vars(model)),
    fn = function(x) gsub("\\)", ") ", x) ) %>%
  tab_options(row_group.font.weight = "bold", 
              row_group.background.color = brewer.pal(9,"Greys")[3], 
...
#Create AIC model selection table with delta AIC < 2
MP_chicks_Phi_run$model.table %>%
  filter(DeltaAICc < 2 | model == "Phi(~1)p(~Quadratic)") %>%
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
...
MP_s_funday_mgmt_l_p_quadage_x_year_surv <- MP_chicks_Phi_run[[42]]$results$real

Groups <- 
  data.frame(str_split_fixed(rownames(MP_s_funday_mgmt_l_p_quadage_x_year_surv), " ", n = 5))
MP_s_funday_mgmt_l_p_quadage_x_year_surv <- 
  cbind(Groups, MP_s_funday_mgmt_l_p_quadage_x_year_surv)
MP_s_funday_mgmt_l_p_quadage_x_year_surv$season <-
  unlist(substr(MP_s_funday_mgmt_l_p_quadage_x_year_surv$X2, 2, 7))
MP_s_funday_mgmt_l_p_quadage_x_year_surv$age <- 
  as.numeric(sub("^a", "", MP_s_funday_mgmt_l_p_quadage_x_year_surv$X4))
MP_s_funday_mgmt_l_p_quadage_x_year_surv$mgmt_s <-
  unlist(substr(MP_s_funday_mgmt_l_p_quadage_x_year_surv$X2, 11, 11))

# MP_Chick_dot_surv <- MP_chicks_Phi_run[[13]]$results$real
# MP_Chick_quadage_surv <- MP_chicks_Phi_run[[23]]$results$real
# MP_Chick_hatch_date_surv <- MP_chicks_Phi_run[[8]]$results$real
# MP_Chick_mgmt_s_surv <- MP_chicks_Phi_run[[20]]$results$real
# MP_Chick_mgmt_l_surv <- MP_chicks_Phi_run[[19]]$results$real
# MP_Chick_mgmt_s_x_quadage_surv <- MP_chicks_Phi_run[[25]]$results$real
# MP_Chick_mgmt_s_quadage_surv <- MP_chicks_Phi_run[[22]]$results$real
# MP_Chick_mgmt_l_quadage_surv <- MP_chicks_Phi_run[[21]]$results$real
# MP_Chick_funday_mgmt_s_surv  <- MP_chicks_Phi_run[[15]]$results$real # second most convincing based on a priori hypothesis and more parsimonious)
# MP_Chick_funday_mgmt_l_surv  <- MP_chicks_Phi_run[[14]]$results$real # TOP MODEL (and most convincing based on a priori hypothesis)
# MP_Chick_funday_surv            <- MP_chicks_Phi_run[[16]]$results$real
# MP_Chick_funday_x_mgmt_s_surv   <- MP_chicks_Phi_run[[18]]$results$real
# MP_Chick_funday_x_mgmt_l_surv   <- MP_chicks_Phi_run[[17]]$results$real

# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_dot_surv), " ", n = 5))
# MP_Chick_dot_surv <- cbind(Groups, MP_Chick_dot_surv)
# MP_Chick_dot_surv$season <- unlist(substr(MP_Chick_dot_surv$X2, 2, 7))
# MP_Chick_dot_surv$age <- as.numeric(sub("^a", "", MP_Chick_dot_surv$X4))
# MP_Chick_dot_surv$mgmt_s <- unlist(substr(MP_Chick_dot_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_quadage_surv), " ", n = 5))
# MP_Chick_quadage_surv <- cbind(Groups, MP_Chick_quadage_surv)
# MP_Chick_quadage_surv$season <- unlist(substr(MP_Chick_quadage_surv$X2, 2, 7))
# MP_Chick_quadage_surv$age <- as.numeric(sub("^a", "", MP_Chick_quadage_surv$X4))
# MP_Chick_quadage_surv$mgmt_s <- unlist(substr(MP_Chick_quadage_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_hatch_date_surv), " ", n = 5))
# MP_Chick_hatch_date_surv <- cbind(Groups, MP_Chick_hatch_date_surv)
# MP_Chick_hatch_date_surv$season <- unlist(substr(MP_Chick_hatch_date_surv$X2, 2, 7))
# MP_Chick_hatch_date_surv$age <- as.numeric(sub("^a", "", MP_Chick_hatch_date_surv$X4))
# MP_Chick_hatch_date_surv$mgmt_s <- unlist(substr(MP_Chick_hatch_date_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_mgmt_s_surv), " ", n = 5))
# MP_Chick_mgmt_s_surv <- cbind(Groups, MP_Chick_mgmt_s_surv)
# MP_Chick_mgmt_s_surv$season <- unlist(substr(MP_Chick_mgmt_s_surv$X2, 2, 7))
# MP_Chick_mgmt_s_surv$age <- as.numeric(sub("^a", "", MP_Chick_mgmt_s_surv$X4))
# MP_Chick_mgmt_s_surv$mgmt_s <- unlist(substr(MP_Chick_mgmt_s_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_mgmt_l_surv), " ", n = 5))
# MP_Chick_mgmt_l_surv <- cbind(Groups, MP_Chick_mgmt_l_surv)
# MP_Chick_mgmt_l_surv$season <- unlist(substr(MP_Chick_mgmt_l_surv$X2, 2, 7))
# MP_Chick_mgmt_l_surv$age <- as.numeric(sub("^a", "", MP_Chick_mgmt_l_surv$X4))
# MP_Chick_mgmt_l_surv$mgmt_l <- unlist(substr(MP_Chick_mgmt_l_surv$X2, 10, 10))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_mgmt_s_quadage_surv), " ", n = 5))
# MP_Chick_mgmt_s_quadage_surv <- cbind(Groups, MP_Chick_mgmt_s_quadage_surv)
# MP_Chick_mgmt_s_quadage_surv$season <- unlist(substr(MP_Chick_mgmt_s_quadage_surv$X2, 2, 7))
# MP_Chick_mgmt_s_quadage_surv$age <- as.numeric(sub("^a", "", MP_Chick_mgmt_s_quadage_surv$X4))
# MP_Chick_mgmt_s_quadage_surv$mgmt_s <- unlist(substr(MP_Chick_mgmt_s_quadage_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_mgmt_s_x_quadage_surv), " ", n = 5))
# MP_Chick_mgmt_s_x_quadage_surv <- cbind(Groups, MP_Chick_mgmt_s_x_quadage_surv)
# MP_Chick_mgmt_s_x_quadage_surv$season <- unlist(substr(MP_Chick_mgmt_s_x_quadage_surv$X2, 2, 7))
# MP_Chick_mgmt_s_x_quadage_surv$age <- as.numeric(sub("^a", "", MP_Chick_mgmt_s_x_quadage_surv$X4))
# MP_Chick_mgmt_s_x_quadage_surv$mgmt_s <- unlist(substr(MP_Chick_mgmt_s_x_quadage_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_mgmt_l_quadage_surv), " ", n = 5))
# MP_Chick_mgmt_l_quadage_surv <- cbind(Groups, MP_Chick_mgmt_l_quadage_surv)
# MP_Chick_mgmt_l_quadage_surv$season <- unlist(substr(MP_Chick_mgmt_l_quadage_surv$X2, 2, 7))
# MP_Chick_mgmt_l_quadage_surv$age <- as.numeric(sub("^a", "", MP_Chick_mgmt_l_quadage_surv$X4))
# MP_Chick_mgmt_l_quadage_surv$mgmt_l <- unlist(substr(MP_Chick_mgmt_l_quadage_surv$X2, 10, 10))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_funday_x_mgmt_s_surv), " ", n = 5))
# MP_Chick_funday_x_mgmt_s_surv <- cbind(Groups, MP_Chick_funday_x_mgmt_s_surv)
# MP_Chick_funday_x_mgmt_s_surv$season <- unlist(substr(MP_Chick_funday_x_mgmt_s_surv$X2, 2, 7))
# MP_Chick_funday_x_mgmt_s_surv$age <- as.numeric(sub("^a", "", MP_Chick_funday_x_mgmt_s_surv$X4))
# MP_Chick_funday_x_mgmt_s_surv$mgmt_l <- unlist(substr(MP_Chick_funday_x_mgmt_s_surv$X2, 10, 10))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_funday_x_mgmt_l_surv), " ", n = 5))
# MP_Chick_funday_x_mgmt_l_surv <- cbind(Groups, MP_Chick_funday_x_mgmt_l_surv)
# MP_Chick_funday_x_mgmt_l_surv$season <- unlist(substr(MP_Chick_funday_x_mgmt_l_surv$X2, 2, 7))
# MP_Chick_funday_x_mgmt_l_surv$age <- as.numeric(sub("^a", "", MP_Chick_funday_x_mgmt_l_surv$X4))
# MP_Chick_funday_x_mgmt_l_surv$mgmt_l <- unlist(substr(MP_Chick_funday_x_mgmt_l_surv$X2, 10, 10))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_funday_mgmt_s_surv), " ", n = 5))
# MP_Chick_funday_mgmt_s_surv <- cbind(Groups, MP_Chick_funday_mgmt_s_surv)
# MP_Chick_funday_mgmt_s_surv$season <- unlist(substr(MP_Chick_funday_mgmt_s_surv$X2, 2, 7))
# MP_Chick_funday_mgmt_s_surv$age <- as.numeric(sub("^a", "", MP_Chick_funday_mgmt_s_surv$X4))
# MP_Chick_funday_mgmt_s_surv$mgmt_s <- unlist(substr(MP_Chick_funday_mgmt_s_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(MP_Chick_funday_surv), " ", n = 5))
# MP_Chick_funday_surv <- cbind(Groups, MP_Chick_funday_surv)
# MP_Chick_funday_surv$season <- unlist(substr(MP_Chick_funday_surv$X2, 2, 7))
# MP_Chick_funday_surv$age <- as.numeric(sub("^a", "", MP_Chick_funday_surv$X4))
# MP_Chick_funday_surv$mgmt_s <- unlist(substr(MP_Chick_funday_surv$X2, 11, 11))
```

### Plot the effect of funday exposure under different managment levels
Our model shows that the probability of daily survival decreases with increasing exposure to fundays, with this effect being dampened by increasing management level. It’s important to note that chicks with high numbers of fundays have survived multiple days already. For example, a chick that has experienced 30 fundays has clearly survived the pressures that earlier chicks did not. In other words, high cumulative fundays do not imply that these chicks are unaffected by human activity; rather, only the chicks that have already survived many days accumulate high funday counts. The negative effect of fundays on daily survival tells us that each additional funday reduces survival probability, even for chicks that have survived a long time.
#### Key technical points to interpretation:
 - The covariate is time-varying and cumulative, so there is a form of conditioning on prior survival.

 - Each beta represents the effect of an additional funday on the hazard of dying (or on daily survival probability), holding the current state of the chick.

 - While high-funday chicks have survived longer, the model is estimating the incremental risk per funday, not the overall survival probability from day 0.
 
```{r}
### plot the survival trend of the Phi(~funday + mgmt_l)p(~Quadratic) model ----
# extract the beta and vcv estimates
funday_mgmt_l_vcv <- 
  MP_chicks_Phi_run[[42]]$results$beta.vcv

funday_mgmt_l_betas <- 
  MP_chicks_Phi_run[[42]]$results$beta

# Assign the correct names to the vcv matrix
rownames(funday_mgmt_l_vcv) <- rownames(funday_mgmt_l_betas)
colnames(funday_mgmt_l_vcv) <- rownames(funday_mgmt_l_betas)

# Define funday gradient
funday_vals <- 
  seq(min(MP_chicks.ddl$Phi$funday), max(MP_chicks.ddl$Phi$funday), length.out = 50)

# mgmt_l levels (assuming 0-5 coded, adjust if different)
mgmt_levels <- paste0("mgmt_l", 0:5)  # adjust 0 if intercept represents mgmt_l0

# Number of simulations
n_sims <- 10000

# --- Function to build design matrix for a given mgmt_l ---
build_design <- function(funday_vals, mgmt_l_level) {
  n <- length(funday_vals)
  
  # Create an empty matrix with same columns as betas
  mat <- matrix(0, nrow = n, ncol = length(funday_mgmt_l_betas$estimate))
  colnames(mat) <- rownames(funday_mgmt_l_betas)
  
  # Fill intercept
  mat[, "Phi:(Intercept)"] <- 1
  
  # Fill funday covariate
  mat[, "Phi:funday"] <- funday_vals
  
  # Fill the mgmt_l dummy
  if (mgmt_l_level != 0) {   # baseline is 0, others add their beta
    mat[, paste0("Phi:mgmt_l", mgmt_l_level)] <- 1
  }
...
# --- Simulate beta draws ---
set.seed(123)
beta_sims <- mvrnorm(n = n_sims, mu = funday_mgmt_l_betas$estimate, Sigma = funday_mgmt_l_vcv)

# --- Compute predicted survival and 95% CIs ---
pred_list <- list()

for (i in 0:5) {  # mgmt_l levels
  design_mat <- build_design(funday_vals, i)
  
  # Linear predictor
  eta <- design_mat %*% t(beta_sims)
  
  # Transform via inverse logit
  Phi_pred <- plogis(eta)
  
  # Compute pointwise summary: mean and 95% CI
  Phi_mean <- apply(Phi_pred, 1, mean)
  Phi_lcl  <- apply(Phi_pred, 1, quantile, probs = 0.025)
  Phi_ucl  <- apply(Phi_pred, 1, quantile, probs = 0.975)
  
  pred_list[[paste0("mgmt_l", i)]] <- data.frame(
    funday = funday_vals,
    mean   = Phi_mean,
    lcl    = Phi_lcl,
    ucl    = Phi_ucl,
...
# Extract two Dark2 colors
cols <- brewer.pal(3, "Dark2")[1:2]  # pick first two colors

MP_chick_ch %>% pull(mgmt_l) %>% table()
# note that I removed the prediction of the management level 2 because it failed to properly estimate this likely due to low sample size (only 4 broods experienced this treatment)

fundays_mgmt_l_brood_surv_plot <- 
  ggplot(pred_df %>% 
         filter(mgmt_l != 2), 
       aes(x = funday, y = mean, color = as.numeric(mgmt_l), fill = as.numeric(mgmt_l))) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays (weekend days and holidays) exposed to", 
       y = "Apparent daily brood survival (± 95% CI)",  color = "Management\nlevel",  # line break with \n
    fill = "Management\nlevel"    # line break with \n
  ) +
  theme_minimal() +
  facet_grid(. ~ mgmt_l) +
  scale_color_gradient(
    low = cols[2],  # from Dark2[2]
    high = cols[1]  # to Dark2[1]
  ) +
  scale_fill_gradient(
    low = cols[2],  # from Dark2[2]
    high = cols[1]  # to Dark2[1]
...
ggsave(plot = fundays_mgmt_l_brood_surv_plot, 
       filename = "fundays_mgmt_l_brood_surv_plot.png", 
       path = "results/", height = 4, width = 8)
```

```{r}
# plotting MP_Chick_funday_surv (NEED TO FIX THIS)
MP_chicks.ddl$Phi$ID <- seq_len(nrow(MP_chicks.ddl$Phi))
MP_Chick_funday_surv$ID <- seq_len(nrow(MP_Chick_funday_surv))

funday_map <- MP_chicks.ddl$Phi[, c("ID", "funday")]

MP_Chick_funday_surv <- merge(MP_Chick_funday_surv, funday_map, by = "ID")
ggplot(MP_Chick_funday_surv, aes(x = funday, y = estimate)) +
  geom_smooth(method = "loess", se = TRUE, color = "#0072B2", fill = "#0072B2", alpha = 0.2) +
  labs(
    x = "Number of fundays a brood is exposed to",
    y = "Daily brood survival (± 95% CI)"
  ) +
  theme_minimal()

# plotting MP_Chick_funday_mgmt_s_surv
MP_chicks.ddl$Phi$ID <- seq_len(nrow(MP_chicks.ddl$Phi))
MP_Chick_funday_mgmt_s_surv$ID <- seq_len(nrow(MP_Chick_funday_mgmt_s_surv))

funday_map <- MP_chicks.ddl$Phi[, c("ID", "funday")]


MP_Chick_funday_mgmt_s_surv <- merge(MP_Chick_funday_mgmt_s_surv, funday_map, by = "ID")
ggplot() +
  geom_smooth(
    data = filter(MP_Chick_funday_mgmt_s_surv, X1 == "Phi"),
    aes(x = funday, y = estimate, color = mgmt_s, fill = mgmt_s),
    method = "loess",    # smooths with local regression
    se = TRUE,           # adds the confidence interval
    alpha = 0.2
  ) +
  labs(
    x = "Number of fundays a brood is exposed to",
...
# constant survival
ggplot() +
  geom_line(data = MP_Chick_dot_surv, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
  geom_ribbon(data = MP_Chick_dot_surv, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
  scale_color_gradient(
    low = "black",
    high = "red",
    name = "season"
  ...
  ylab("Daily brood survival (± 95% CI)") +
  xlab("Brood age (days)")

# quadratic age-dependent survival
ggplot() +
  geom_line(data = filter(MP_Chick_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate)) +
  geom_ribbon(data = filter(MP_Chick_quadage_surv, X1 == "Phi"), 
...
ggplot() +
  geom_line(data = filter(MP_Chick_mgmt_s_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_s)) +
  geom_ribbon(data = filter(MP_Chick_mgmt_s_quadage_surv, X1 == "Phi"), 
...
ggplot() +
  geom_line(data = filter(MP_Chick_mgmt_l_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_l)) +
  geom_ribbon(data = filter(MP_Chick_mgmt_l_quadage_surv, X1 == "Phi"), 
...
# quadratic age-dependent survival and management status
ggplot() +
  geom_line(data = filter(MP_Chick_mgmt_s_x_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_s)) +
  geom_ribbon(data = filter(MP_Chick_mgmt_s_x_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  ylab("Daily brood survival (± 95% CI)") +
  xlab("Brood age (days)")
...
ggplot() +
  geom_point(data = filter(MP_Chick_mgmt_s_surv, X1 == "Phi"), 
            aes(x = mgmt_s, y = estimate, color = mgmt_s)) +
  geom_errorbar(data = filter(MP_Chick_mgmt_s_surv, X1 == "Phi"), 
...
ggplot() +
  geom_point(data = filter(MP_Chick_mgmt_l_surv, X1 == "Phi"), 
            aes(x = mgmt_l, y = estimate, color = mgmt_l)) +
  geom_errorbar(data = filter(MP_Chick_mgmt_l_surv, X1 == "Phi"), 
              aes(x = mgmt_l, ymin = lcl, ymax = ucl, color = mgmt_l)) +
  ylab("Daily brood survival (± 95% CI)") +
  xlab("Managed?") +
  scale_y_continuous(limits = c(0, 1))

filter(MP_Chick_mgmt_s_surv, X1 == "Phi") %>% 
  mutate(fledgling_success = estimate^35)

# seasonal dependent survival (hatch date)
# ggplot() +
#   geom_line(data = MP_Chick_p_reals, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
#   geom_ribbon(data = MP_Chick_p_reals, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
#   scale_color_gradient(
#     low = "black",
#     high = "red",
#     name = "season"
...
BC_chick_ch <- 
  BC_chick_ch %>% 
  mutate(mgmt_45 = ifelse(mgmt_l %in% c("4", "5"), "Y", "N"))

# Create processed RMARK data format as CJS with "Year" as group
BC_chicks_45.proc <- process.data(BC_chick_ch, model = "CJS",
                               groups = c("season", "region", 
                                          "mgmt_l", "mgmt_s", "mgmt_45"))

# Create the design data
BC_chicks_45.ddl <- make.design.data(BC_chicks_45.proc)

# create a quadratic time variable
time <- c(0:(BC_chicks_45.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(BC_chicks_45.proc$nocc[1]-1),
                         funday = BC_chick_45_ch$funday[1:BC_chicks_45.proc$nocc[1]])
BC_chicks_45.ddl$p <- merge_design.covariates(BC_chicks_45.ddl$Phi, 
                                              quad_time, bygroup = FALSE, bytime = TRUE)
BC_chicks_45.ddl$Phi <- merge_design.covariates(BC_chicks_45.ddl$Phi, 
                                                quad_time, bygroup = FALSE, bytime = TRUE)
BC_chicks_45.ddl$Phi <- merge_design.covariates(BC_chicks_45.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```
...
BC_chicks_45_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = BC_chicks_45.proc, 
                             ddl = BC_chicks_45.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
  
  # store coBCleted model list
  return(model.list) 
}

# Run model selection to determine best structure of p
BC_chicks_45_p_run <- BC_chicks_45_p()

# inspect p AIC table to find top model
BC_chicks_45_p_run # best structure: p(~1) #model 3

saveRDS(BC_chicks_45_p_run, "output/BC_chicks_45_p_run.rds")

#Create AIC model selection table

BC_chicks_45_p_run <- readRDS(file = "output/BC_chicks_45_p_run.rds") 

BC_chicks_45_p_run$model.table %>% 
  mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "time",
...
### Extract results and visualize p
```{r}
BC_chicks_p_run <- readRDS("output/BC_chicks_p_run.rds")
#This is the second model not the top model
# Extract estimates of survival from top model
Chick_p_reals_BC <- BC_chicks_p_run[[4]]$results$real

# wrangle dataframe to get annual estimates (grouped by "Year")
Groups <- data.frame(str_split_fixed(rownames(Chick_p_reals_BC), " ", n = 5))
Chick_p_reals_BC <- cbind(Groups, Chick_p_reals_BC)
Chick_p_reals_BC$season <- unlist(substr(Chick_p_reals_BC$X2, 2, 7))
Chick_p_reals_BC$age <- as.numeric(sub("^a", "", Chick_p_reals_BC$X4))

# plot of the quadratic age dependent relationship with detection probability
ggplot() +
  geom_line(data = Chick_p_reals_BC %>% filter(X1 == "p"), aes(x = age, y = estimate)) +
  geom_ribbon(data = Chick_p_reals_BC %>% filter(X1 == "p"), aes(x = age, ymin = lcl, ymax = ucl), alpha = 0.2) +
  ylab("daily detection probability of a brood") +
  xlab("brood age (days)")
```
### Survival modelling (Phi)
...
BC_chicks_Phi <- function() 
{
  ...
  Phi.quadage.mgmt_s <-
    ...
    # interaction between hatch date and age and management status
  Phi.date.mgmt_45 <-
    list(formula = ~date_j + mgmt_45)

  # interaction between hatch date and quadratic age and management status
  Phi.age.mgmt_45 <-
    list(formula = ~Time + mgmt_45)

  # interaction between year and hatch date and management status
  Phi.quadage.mgmt_45 <-
    list(formula = ~Quadratic + mgmt_45)
...
# Run model selection to determine best structure of Phi
BC_chicks_Phi_run <- BC_chicks_Phi()

# inspect Phi AIC table to find top model
BC_chicks_Phi_run 

saveRDS(BC_chicks_Phi_run, "output/BC_chicks_Phi_run.rds")

#Create AIC model selection table

BC_chicks_Phi_run <- readRDS(file = "output/BC_chicks_Phi_run.rds") 

BC_chicks_Phi_run$model.table %>% 
    mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
...
### Extract results and visualize Phi
```{r}
BC_chicks_Phi_run <- readRDS("output/BC_chicks_Phi_run.rds")

# Extract estimates of survival from top model (the constant model is the 
# most parsimonious, and so let's go with that one)
BC_Chick_dot_surv <- BC_chicks_Phi_run[[51]]$results$real

Groups <- data.frame(str_split_fixed(rownames(BC_Chick_dot_surv), " ", n = 5))
BC_Chick_dot_surv <- cbind(Groups, BC_Chick_dot_surv)
BC_Chick_dot_surv$season <- unlist(substr(BC_Chick_dot_surv$X2, 2, 7))
BC_Chick_dot_surv$age <- as.numeric(sub("^a", "", BC_Chick_dot_surv$X4))
BC_Chick_dot_surv$mgmt_l <- unlist(substr(BC_Chick_dot_surv$X2, 10, 10))

constant_plot_BC <- 
  ggplot() +
  geom_errorbar(data = BC_Chick_dot_surv %>% filter(X1 == "Phi"), 
                aes(ymin = lcl, ymax = ucl, 
                    x = 1, 
                    y = estimate),
                alpha = 0.5, color = "black", width = 0.1, lwd = 0.5) +
  geom_point(data = BC_Chick_dot_surv %>% filter(X1 == "Phi"), 
             aes(x = 1, y = estimate),
             shape = 21, size = 4, fill = "grey80") +
  scale_colour_brewer(palette = "Set1") +
  luke_theme +
  theme(legend.position = c(0.55, 0.05),
        legend.justification = c(1, 0),
...
ggsave(plot = constant_plot_BC, 
       filename = "constant_plot_BC.png", 
       path = "results/", height = 4, width = 4)
```
...
BSC_chick_ch <- BSC_chick_ch[nchar(BSC_chick_ch$ch) == 35, ] #two brood had "ch" observations greater than 35 so i removed them

# Create processed RMARK data format as CJS with "Year" as group
BSC_chicks.proc <- process.data(BSC_chick_ch, model = "CJS",
                               groups = c("season", "region", 
                                          "mgmt_l", "mgmt_s"))
    
# Create the design data
BSC_chicks.ddl <- make.design.data(BSC_chicks.proc)

# create a quadratic time variable
time <- c(0:(BSC_chicks.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(BSC_chicks.proc$nocc[1]-1),
                         funday = BSC_chick_ch$funday[1:BSC_chicks.proc$nocc[1]])
BSC_chicks.ddl$p <- merge_design.covariates(BSC_chicks.ddl$Phi, 
                                              quad_time, bygroup = FALSE, bytime = TRUE)
BSC_chicks.ddl$Phi <- merge_design.covariates(BSC_chicks.ddl$Phi, 
                                                quad_time, bygroup = FALSE, bytime = TRUE)
BSC_chicks.ddl$Phi <- merge_design.covariates(BSC_chicks.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```
...
BSC_chicks_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = BSC_chicks.proc, 
                             ddl = BSC_chicks.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
  
  # store coBSCleted model list
  return(model.list) 
}

# Run model selection to determine best structure of p
BSC_chicks_p_run <- BSC_chicks_p()

# inspect p AIC table to find top model
BSC_chicks_p_run # best structure: p(~season * date_j)

saveRDS(BSC_chicks_p_run, "output/BSC_chicks_p_run.rds")

#Create AIC model selection table

BSC_chicks_p_run <- readRDS(file = "output/BSC_chicks_p_run.rds") 

BSC_chicks_p_run$model.table %>% 
  mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "time",
...
### Extract results and visualize p
```{r}
BSC_chicks_p_run <- readRDS("output/BSC_chicks_p_run.rds")

# Extract estimates of survival from top model
Chick_p_reals_BSC <- BSC_chicks_p_run[10]$results$real

# wrangle dataframe to get annual estimates (grouped by "Year")
Groups <- data.frame(str_split_fixed(rownames(Chick_p_reals_BSC), " ", n = 5))
Chick_p_reals_BSC <- cbind(Groups, Chick_p_reals_BSC)
Chick_p_reals_BSC$season <- unlist(substr(Chick_p_reals_BSC$X2, 2, 7))
Chick_p_reals_BSC$age <- as.numeric(sub("^a", "", Chick_p_reals_BSC$X4))

ggplot() +
  geom_line(data = Chick_p_reals_BSC, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
  geom_ribbon(data = Chick_p_reals_BSC, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
  scale_color_gradient(
    low = "black",
    high = "red",
    name = "season"
...
Chick_p_reals_BSC <- Chick_p_reals_BSC[which(Chick_p_reals_BSC$X1 == "p"),]

# clean dataframe
Chick_p_reals_BSC <- Chick_p_reals_BSC[,-c(1:11)]
Chick_p_reals_BSC$Age <- "Chick"
row.names(Chick_p_reals_BSC) <- NULL
```
### Survival modelling (Phi)
```{r}
# Find best structure of Phi while using p(~Year * Day_of_Season)
BSC_chicks_Phi <- function() 
{
  # Constant Phi
  Phi.dot <- 
    list(formula = ~1)
...
# Run model selection to determine best structure of Phi
BSC_chicks_Phi_run <- BSC_chicks_Phi()

# inspect Phi AIC table to find top model
BSC_chicks_Phi_run 

saveRDS(BSC_chicks_Phi_run, "output/BSC_chicks_Phi_run.rds")

#Create AIC model selection table

BSC_chicks_Phi_run <- readRDS(file = "output/BSC_chicks_Phi_run.rds") 

BSC_chicks_Phi_run$model.table %>% 
 mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
  ...
  cols_align(align = "left", columns = vars(model)) %>% 
   tab_style(style = cell_text(weight = "bold"),
    locations = cells_body(
      columns = vars(model),
      rows = model == "Phi(~1)p(~year * hatch date)")) %>% 
   text_transform(locations = cells_body(columns = vars(model)),
    fn = function(x) gsub("\\)", ") ", x) ) %>%
  tab_options(row_group.font.weight = "bold", 
              row_group.background.color = brewer.pal(9,"Greys")[3], 
...
#Create AIC model selection table with delta AIC < 2
BSC_chicks_Phi_run$model.table %>%
  filter(DeltaAICc < 2 | model == "Phi(~1)p(~season * date_j)") %>%
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
...
### Extract results and visualize Phi
```{r}
BSC_chicks_Phi_run <- readRDS("output/BSC_chicks_Phi_run.rds")

# Extract estimates of survival from top models
#BSC_Chick_dot_surv <- BSC_chicks_Phi_run[[13]]$results$real
# BSC_Chick_quadage_surv <- BSC_chicks_Phi_run[[23]]$results$real
#BSC_Chick_hatch_date_surv <- BSC_chicks_Phi_run[[1]]$results$real
#BSC_Chick_mgmt_s_surv <- BSC_chicks_Phi_run[[15]]$results$real
#BSC_Chick_mgmt_l_surv <- BSC_chicks_Phi_run[[22]]$results$real
#BSC_Chick_mgmt_s_x_quadage_surv <- BSC_chicks_Phi_run[[20]]$results$real
# BSC_Chick_mgmt_s_quadage_surv <- BSC_chicks_Phi_run[[22]]$results$real
BSC_Chick_mgmt_l_quadage_surv <- BSC_chicks_Phi_run[[21]]$results$real


# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_dot_surv), " ", n = 5))
# BSC_Chick_dot_surv <- cbind(Groups, BSC_Chick_dot_surv)
# BSC_Chick_dot_surv$season <- unlist(substr(BSC_Chick_dot_surv$X2, 2, 7))
# BSC_Chick_dot_surv$age <- as.numeric(sub("^a", "", BSC_Chick_dot_surv$X4))
# BSC_Chick_dot_surv$mgmt_s <- unlist(substr(BSC_Chick_dot_surv$X2, 12, 12))
# 
# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_quadage_surv), " ", n = 5))
# BSC_Chick_quadage_surv <- cbind(Groups, BSC_Chick_quadage_surv)
# BSC_Chick_quadage_surv$season <- unlist(substr(BSC_Chick_quadage_surv$X2, 2, 7))
# BSC_Chick_quadage_surv$age <- as.numeric(sub("^a", "", BSC_Chick_quadage_surv$X4))
# BSC_Chick_quadage_surv$mgmt_s <- unlist(substr(BSC_Chick_quadage_surv$X2, 12, 12))
# 
# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_hatch_date_surv), " ", n = 5))
# BSC_Chick_hatch_date_surv <- cbind(Groups, BSC_Chick_hatch_date_surv)
# BSC_Chick_hatch_date_surv$season <- unlist(substr(BSC_Chick_hatch_date_surv$X2, 2, 7))
# BSC_Chick_hatch_date_surv$age <- as.numeric(sub("^a", "", BSC_Chick_hatch_date_surv$X4))
# BSC_Chick_hatch_date_surv$mgmt_s <- unlist(substr(BSC_Chick_hatch_date_surv$X2, 12, 12))
# 
# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_mgmt_s_surv), " ", n = 5))
# BSC_Chick_mgmt_s_surv <- cbind(Groups, BSC_Chick_mgmt_s_surv)
# BSC_Chick_mgmt_s_surv$season <- unlist(substr(BSC_Chick_mgmt_s_surv$X2, 2, 7))
# BSC_Chick_mgmt_s_surv$age <- as.numeric(sub("^a", "", BSC_Chick_mgmt_s_surv$X4))
# BSC_Chick_mgmt_s_surv$mgmt_s <- unlist(substr(BSC_Chick_mgmt_s_surv$X2, 12, 12))
# 
# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_mgmt_l_surv), " ", n = 5))
# BSC_Chick_mgmt_l_surv <- cbind(Groups, BSC_Chick_mgmt_l_surv)
# BSC_Chick_mgmt_l_surv$season <- unlist(substr(BSC_Chick_mgmt_l_surv$X2, 2, 7))
# BSC_Chick_mgmt_l_surv$age <- as.numeric(sub("^a", "", BSC_Chick_mgmt_l_surv$X4))
# BSC_Chick_mgmt_l_surv$mgmt_l <- unlist(substr(BSC_Chick_mgmt_l_surv$X2, 11, 11))
# 
# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_mgmt_s_quadage_surv), " ", n = 5))
# BSC_Chick_mgmt_s_quadage_surv <- cbind(Groups, BSC_Chick_mgmt_s_quadage_surv)
# BSC_Chick_mgmt_s_quadage_surv$season <- unlist(substr(BSC_Chick_mgmt_s_quadage_surv$X2, 2, 7))
# BSC_Chick_mgmt_s_quadage_surv$age <- as.numeric(sub("^a", "", BSC_Chick_mgmt_s_quadage_surv$X4))
# BSC_Chick_mgmt_s_quadage_surv$mgmt_s <- unlist(substr(BSC_Chick_mgmt_s_quadage_surv$X2, 12, 12))
# 
# Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_mgmt_s_x_quadage_surv), " ", n = 5))
# BSC_Chick_mgmt_s_x_quadage_surv <- cbind(Groups, BSC_Chick_mgmt_s_x_quadage_surv)
# BSC_Chick_mgmt_s_x_quadage_surv$season <- unlist(substr(BSC_Chick_mgmt_s_x_quadage_surv$X2, 2, 7))
# BSC_Chick_mgmt_s_x_quadage_surv$age <- as.numeric(sub("^a", "", BSC_Chick_mgmt_s_x_quadage_surv$X4))
# BSC_Chick_mgmt_s_x_quadage_surv$mgmt_s <- unlist(substr(BSC_Chick_mgmt_s_x_quadage_surv$X2, 12, 12))

Groups <- data.frame(str_split_fixed(rownames(BSC_Chick_mgmt_l_quadage_surv), " ", n = 5))
BSC_Chick_mgmt_l_quadage_surv <- cbind(Groups, BSC_Chick_mgmt_l_quadage_surv)
BSC_Chick_mgmt_l_quadage_surv$season <- unlist(substr(BSC_Chick_mgmt_l_quadage_surv$X2, 2, 7))
BSC_Chick_mgmt_l_quadage_surv$age <- as.numeric(sub("^a", "", BSC_Chick_mgmt_l_quadage_surv$X4))
BSC_Chick_mgmt_l_quadage_surv$mgmt_l <- unlist(substr(BSC_Chick_mgmt_l_quadage_surv$X2, 11, 11))

BSC_Chick_mgmt_l_quadage_surv_plot <-
  ggplot(BSC_Chick_mgmt_l_quadage_surv %>% 
         filter(mgmt_l != 0) %>%
           filter(X1 == "Phi"), 
       aes(x = age, y = estimate, 
           color = as.numeric(mgmt_l), 
           fill = as.numeric(mgmt_l))) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Brood age (days)", 
       y = "Daily brood survival (± 95% CI)",
       color = "Management\nlevel",  # line break with \n
    fill = "Management\nlevel"    # line break with \n
  ) +
...
ggsave(plot = BSC_Chick_mgmt_l_quadage_surv_plot, 
       filename = "BSC_Chick_mgmt_l_quadage_surv_plot.png", 
       path = "results/", height = 4, width = 8)


# Get the colors that ggplot would assign automatically to the present levels
present_levels <- levels(factor(BSC_Chick_mgmt_l_quadage_surv$mgmt_l))
color_mapping <- scales::hue_pal()(length(present_levels))
names(color_mapping) <- present_levels
color_mapping

ggplot() +
  geom_line(
    data = filter(BSC_Chick_mgmt_l_quadage_surv, X1 == "Phi"), 
    aes(x = age, y = estimate, color = mgmt_l)) +
  geom_ribbon(
    data = filter(BSC_Chick_mgmt_l_quadage_surv, X1 == "Phi"), 
    aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_l),
    alpha = 0.2) +
  scale_color_manual(
...
# quadratic age-dependent survival and management status
ggplot() +
  geom_line(data = filter(BSC_Chick_mgmt_s_x_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_s)) +
  geom_ribbon(data = filter(BSC_Chick_mgmt_s_x_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("brood age (days)")
...
# management status-dependent survival
ggplot() +
  geom_point(data = filter(BSC_Chick_mgmt_s_surv, X1 == "Phi"), 
            aes(x = mgmt_s, y = estimate, color = mgmt_s)) +
  geom_errorbar(data = filter(BSC_Chick_mgmt_s_surv, X1 == "Phi"), 
              aes(x = mgmt_s, ymin = lcl, ymax = ucl, color = mgmt_s)) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("managed?") +
  scale_y_continuous(limits = c(0, 1))
...
# management level-dependent survival
ggplot() +
  geom_point(data = filter(BSC_Chick_mgmt_l_surv, X1 == "Phi"), 
            aes(x = mgmt_l, y = estimate, color = mgmt_l)) +
  geom_errorbar(data = filter(BSC_Chick_mgmt_l_surv, X1 == "Phi"), 
              aes(x = mgmt_l, ymin = lcl, ymax = ucl, color = mgmt_l)) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("managed?") +
  scale_y_continuous(limits = c(0, 1))
...
```{r}
#Create processed RMARK data format as CJS with "Year" as group
FP_chicks.proc <- process.data(FP_chick_ch, model = "CJS",
                               groups = c("season", "region", 
                                          "mgmt_l", "mgmt_s"))

# Create the design data
FP_chicks.ddl <- make.design.data(FP_chicks.proc)

# create a quadratic time variable
time <- c(0:(FP_chicks.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(FP_chicks.proc$nocc[1]-1),
                         funday = FP_chick_ch$funday[1:FP_chicks.proc$nocc[1]])
FP_chicks.ddl$p <- merge_design.covariates(FP_chicks.ddl$Phi, 
                                              quad_time, bygroup = FALSE, bytime = TRUE)
FP_chicks.ddl$Phi <- merge_design.covariates(FP_chicks.ddl$Phi, 
                                                quad_time, bygroup = FALSE, bytime = TRUE)
FP_chicks.ddl$Phi <- merge_design.covariates(FP_chicks.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
...
FP_chicks_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = FP_chicks.proc, 
                             ddl = FP_chicks.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
  
  # store coFPleted model list
  return(model.list) 
}

# Run model selection to determine best structure of p
FP_chicks_p_run <- FP_chicks_p()

# inspect p AIC table to find top model
FP_chicks_p_run # best structure: p(~season + Quadratic)

saveRDS(FP_chicks_p_run, "output/FP_chicks_p_run.rds")


#Create AIC model selection table

FP_chicks_p_run <- readRDS(file = "output/FP_chicks_p_run.rds") 

FP_chicks_p_run$model.table %>% 
  mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "time",
...
### Extract results and visualize p
```{r}
FP_chicks_p_run <- readRDS("output/FP_chicks_p_run.rds")

# Extract estimates of survival from top model
FP_Chick_p_reals <- cbind(Groups, FP_Chick_p_reals)
FP_Chick_p_reals$season <- unlist(substr(FP_Chick_p_reals$X2, 2, 7))
FP_Chick_p_reals$age <- as.numeric(sub("^a", "", FP_Chick_p_reals$X4))

ggplot() +
  geom_line(data = FP_Chick_p_reals, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
  geom_ribbon(data = FP_Chick_p_reals, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
  scale_color_gradient(
    low = "black",
    high = "red",
    name = "season"
...
FP_Chick_p_reals <- FP_Chick_p_reals[which(FP_Chick_p_reals$X1 == "p"),]

# clean dataframe
FP_Chick_p_reals <- FP_Chick_p_reals[,-c(1:11)]
FP_Chick_p_reals$Age <- "Chick"
row.names(FP_Chick_p_reals) <- NULL
```
### Survival modelling (Phi)
```{r}
# Find best structure of Phi while using p(~1))
FP_chicks_Phi <- function() 
{
  # Constant Phi
  Phi.dot <- 
    list(formula = ~1)
...
# Run model selection to determine best structure of Phi
FP_chicks_Phi_run <- FP_chicks_Phi()

# inspect Phi AIC table to find top model
FP_chicks_Phi_run # best structure: Phi(~date_j + management level)

saveRDS(FP_chicks_Phi_run, "output/FP_chicks_Phi_run.rds")

#Create AIC model selection table

FP_chicks_Phi_run <- readRDS(file = "output/FP_chicks_Phi_run.rds") 

FP_chicks_Phi_run$model.table %>% 
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
...
### Extract results and visualize Phi
```{r}
FP_chicks_Phi_run <- readRDS("output/FP_chicks_Phi_run.rds")

# Extract estimates of survival from top models
FP_Chick_dot_surv <- FP_chicks_Phi_run[[13]]$results$real
FP_Chick_quadage_surv <- FP_chicks_Phi_run[[23]]$results$real
FP_Chick_hatch_date_surv <- FP_chicks_Phi_run[[8]]$results$real
FP_Chick_mgmt_s_surv <- FP_chicks_Phi_run[[20]]$results$real
FP_Chick_mgmt_l_surv <- FP_chicks_Phi_run[[19]]$results$real
FP_Chick_mgmt_s_x_quadage_surv <- FP_chicks_Phi_run[[25]]$results$real
FP_Chick_mgmt_s_quadage_surv <- FP_chicks_Phi_run[[22]]$results$real
FP_Chick_mgmt_l_quadage_surv <- FP_chicks_Phi_run[[21]]$results$real
FP_Chick_hatch_date_mgmt_l_surv <- FP_chicks_Phi_run[[6]]$results$real # top model
FP_Chick_hatch_date_x_mgmt_l_surv <- FP_chicks_Phi_run[[10]]$results$real # second model. only two models <2 Delta AIC

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_hatch_date_mgmt_l_surv), " ", n = 5))
FP_Chick_hatch_date_mgmt_l_surv <- cbind(Groups, FP_Chick_hatch_date_mgmt_l_surv)
FP_Chick_hatch_date_mgmt_l_surv$season <- unlist(substr(FP_Chick_hatch_date_mgmt_l_surv$X2, 2, 7))
FP_Chick_hatch_date_mgmt_l_surv$age <- as.numeric(sub("^a", "", FP_Chick_hatch_date_mgmt_l_surv$X4))
FP_Chick_hatch_date_mgmt_l_surv$mgmt_s <- unlist(substr(FP_Chick_hatch_date_mgmt_l_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_hatch_date_x_mgmt_l_surv), " ", n = 5))
FP_Chick_hatch_date_x_mgmt_l_surv <- cbind(Groups, FP_Chick_hatch_date_x_mgmt_l_surv)
FP_Chick_hatch_date_x_mgmt_l_surv$season <- unlist(substr(FP_Chick_hatch_date_x_mgmt_l_surv$X2, 2, 7))
FP_Chick_hatch_date_x_mgmt_l_surv$age <- as.numeric(sub("^a", "", FP_Chick_hatch_date_x_mgmt_l_surv$X4))
FP_Chick_hatch_date_x_mgmt_l_surv$mgmt_s <- unlist(substr(FP_Chick_hatch_date_x_mgmt_l_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_dot_surv), " ", n = 5))
FP_Chick_dot_surv <- cbind(Groups, FP_Chick_dot_surv)
FP_Chick_dot_surv$season <- unlist(substr(FP_Chick_dot_surv$X2, 2, 7))
FP_Chick_dot_surv$age <- as.numeric(sub("^a", "", FP_Chick_dot_surv$X4))
FP_Chick_dot_surv$mgmt_s <- unlist(substr(FP_Chick_dot_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_quadage_surv), " ", n = 5))
FP_Chick_quadage_surv <- cbind(Groups, FP_Chick_quadage_surv)
FP_Chick_quadage_surv$season <- unlist(substr(FP_Chick_quadage_surv$X2, 2, 7))
FP_Chick_quadage_surv$age <- as.numeric(sub("^a", "", FP_Chick_quadage_surv$X4))
FP_Chick_quadage_surv$mgmt_s <- unlist(substr(FP_Chick_quadage_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_hatch_date_surv), " ", n = 5))
FP_Chick_hatch_date_surv <- cbind(Groups, FP_Chick_hatch_date_surv)
FP_Chick_hatch_date_surv$season <- unlist(substr(FP_Chick_hatch_date_surv$X2, 2, 7))
FP_Chick_hatch_date_surv$age <- as.numeric(sub("^a", "", FP_Chick_hatch_date_surv$X4))
FP_Chick_hatch_date_surv$mgmt_s <- unlist(substr(FP_Chick_hatch_date_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_mgmt_s_surv), " ", n = 5))
FP_Chick_mgmt_s_surv <- cbind(Groups, FP_Chick_mgmt_s_surv)
FP_Chick_mgmt_s_surv$season <- unlist(substr(FP_Chick_mgmt_s_surv$X2, 2, 7))
FP_Chick_mgmt_s_surv$age <- as.numeric(sub("^a", "", FP_Chick_mgmt_s_surv$X4))
FP_Chick_mgmt_s_surv$mgmt_s <- unlist(substr(FP_Chick_mgmt_s_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_mgmt_l_surv), " ", n = 5))
FP_Chick_mgmt_l_surv <- cbind(Groups, FP_Chick_mgmt_l_surv)
FP_Chick_mgmt_l_surv$season <- unlist(substr(FP_Chick_mgmt_l_surv$X2, 2, 7))
FP_Chick_mgmt_l_surv$age <- as.numeric(sub("^a", "", FP_Chick_mgmt_l_surv$X4))
FP_Chick_mgmt_l_surv$mgmt_l <- unlist(substr(FP_Chick_mgmt_l_surv$X2, 10, 10))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_mgmt_s_quadage_surv), " ", n = 5))
FP_Chick_mgmt_s_quadage_surv <- cbind(Groups, FP_Chick_mgmt_s_quadage_surv)
FP_Chick_mgmt_s_quadage_surv$season <- unlist(substr(FP_Chick_mgmt_s_quadage_surv$X2, 2, 7))
FP_Chick_mgmt_s_quadage_surv$age <- as.numeric(sub("^a", "", FP_Chick_mgmt_s_quadage_surv$X4))
FP_Chick_mgmt_s_quadage_surv$mgmt_s <- unlist(substr(FP_Chick_mgmt_s_quadage_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_mgmt_s_x_quadage_surv), " ", n = 5))
FP_Chick_mgmt_s_x_quadage_surv <- cbind(Groups, FP_Chick_mgmt_s_x_quadage_surv)
FP_Chick_mgmt_s_x_quadage_surv$season <- unlist(substr(FP_Chick_mgmt_s_x_quadage_surv$X2, 2, 7))
FP_Chick_mgmt_s_x_quadage_surv$age <- as.numeric(sub("^a", "", FP_Chick_mgmt_s_x_quadage_surv$X4))
FP_Chick_mgmt_s_x_quadage_surv$mgmt_s <- unlist(substr(FP_Chick_mgmt_s_x_quadage_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(FP_Chick_mgmt_l_quadage_surv), " ", n = 5))
FP_Chick_mgmt_l_quadage_surv <- cbind(Groups, FP_Chick_mgmt_l_quadage_surv)
FP_Chick_mgmt_l_quadage_surv$season <- unlist(substr(FP_Chick_mgmt_l_quadage_surv$X2, 2, 7))
FP_Chick_mgmt_l_quadage_surv$age <- as.numeric(sub("^a", "", FP_Chick_mgmt_l_quadage_surv$X4))
FP_Chick_mgmt_l_quadage_surv$mgmt_l <- unlist(substr(FP_Chick_mgmt_l_quadage_surv$X2, 10, 10))

# constant survival
ggplot() +
  geom_line(data = FP_Chick_dot_surv, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
  geom_ribbon(data = FP_Chick_dot_surv, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
  scale_color_gradient(
    low = "black",
    high = "red",
    name = "season"
...
# quadratic age-dependent survival
ggplot() +
  geom_line(data = filter(FP_Chick_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate)) +
  geom_ribbon(data = filter(FP_Chick_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl), alpha = 0.2) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("brood age (days)")
...
# quadratic age-dependent survival and management status
ggplot() +
  geom_line(data = filter(FP_Chick_mgmt_s_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_s)) +
  geom_ribbon(data = filter(FP_Chick_mgmt_s_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("brood age (days)")
...
# quadratic age-dependent survival and management level
ggplot() +
  geom_line(data = filter(FP_Chick_mgmt_l_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_l)) +
  geom_ribbon(data = filter(FP_Chick_mgmt_l_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_l), alpha = 0.2) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("brood age (days)")
...
# quadratic age-dependent survival and management status
ggplot() +
  geom_line(data = filter(FP_Chick_mgmt_s_x_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_s)) +
  geom_ribbon(data = filter(FP_Chick_mgmt_s_x_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("brood age (days)")
...
# management status-dependent survival
ggplot() +
 geom_point(data = filter(FP_Chick_mgmt_s_surv, X1 == "Phi"), 
            aes(x = mgmt_s, y = estimate, color = mgmt_s)) +
  geom_errorbar(data = filter(FP_Chick_mgmt_s_surv, X1 == "Phi"), 
              aes(x = mgmt_s, ymin = lcl, ymax = ucl, color = mgmt_s)) +
 ylab("daily brood survival (± 95% CI)") +
  xlab("managed?") +
  scale_y_continuous(limits = c(0, 1))
...
# management level-dependent survival
ggplot() +
  geom_point(data = filter(FP_Chick_mgmt_l_surv, X1 == "Phi"), 
            aes(x = mgmt_l, y = estimate, color = mgmt_l)) +
  geom_errorbar(data = filter(FP_Chick_mgmt_l_surv, X1 == "Phi"), 
              aes(x = mgmt_l, ymin = lcl, ymax = ucl, color = mgmt_l)) +
  ylab("daily brood survival (± 95% CI)") +
  xlab("managed?") +
 scale_y_continuous(limits = c(0, 1))
...
filter(FP_Chick_mgmt_s_surv, X1 == "Phi") %>% 
  mutate(fledgling_success = estimate^35)


filter(FP_Chick_hatch_date_mgmt_l_surv, X1 == "Phi") %>% #fledging success for top model
  mutate(fledgling_success = estimate^35)
...
```{r}
# Create processed RMARK data format as CJS with "Year" as group
PI_chicks.proc <- process.data(PI_chick_ch, model = "CJS",
                               groups = c("season", "region", 
                                          "mgmt_l", "mgmt_s"))

# Create the design data
PI_chicks.ddl <- make.design.data(PI_chicks.proc)

# create a quadratic time variable
time <- c(0:(PI_chicks.proc$nocc[1]-1))
Quadratic <- time^2
Cubic <- time^3
quad_time <- data.frame(time, Quadratic, Cubic)
funday_cov <- data.frame(time = 0:(PI_chicks.proc$nocc[1]-1),
                         funday = PI_chick_ch$funday[1:PI_chicks.proc$nocc[1]])
PI_chicks.ddl$p <- merge_design.covariates(PI_chicks.ddl$Phi, 
                                              quad_time, bygroup = FALSE, bytime = TRUE)
PI_chicks.ddl$Phi <- merge_design.covariates(PI_chicks.ddl$Phi, 
                                                quad_time, bygroup = FALSE, bytime = TRUE)
PI_chicks.ddl$Phi <- merge_design.covariates(PI_chicks.ddl$Phi, 
                                             funday_cov, bygroup = FALSE, bytime = TRUE)
```
...
PI_chicks_p=function() 
{
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = PI_chicks.proc, 
                             ddl = PI_chicks.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
  
  # store coPIleted model list
  return(model.list) 
}

# Run model selection to determine best structure of p
PI_chicks_p_run <- PI_chicks_p()

# inspect p AIC table to find top model
PI_chicks_p_run # best structure: p(~season + date_j)

saveRDS(PI_chicks_p_run, "output/PI_chicks_p_run.rds")

#Create AIC model selection table

PI_chicks_p_run <- readRDS(file = "output/PI_chicks_p_run.rds") 

PI_chicks_p_run$model.table %>% 
 mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "time",
...
### Extract results and visualize p
```{r}
PI_chicks_p_run <- readRDS("output/PI_chicks_p_run.rds")

# Extract estimates of survival from top model
Chick_p_reals <- PI_chicks_p_run[[4]]$results$real
...
### Survival modelling (Phi)
```{r}
# Find best structure of Phi while using p = ~season * date_j
PI_chicks_Phi <- function() 
{
  # Constant Phi
  Phi.dot <- 
    list(formula = ~1)
...
# Run model selection to determine best structure of Phi
PI_chicks_Phi_run <- PI_chicks_Phi()

# inspect Phi AIC table to find top model
PI_chicks_Phi_run # best structure: Phi(~Year * Day_of_Season)

saveRDS(PI_chicks_Phi_run, "output/PI_chicks_Phi_run.rds")

#Create AIC model selection table

PI_chicks_Phi_run <- readRDS(file = "output/PI_chicks_Phi_run.rds") 

PI_chicks_Phi_run$model.table %>% 
 mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
...
### Extract results and visualize Phi
```{r}
PI_chicks_Phi_run <- readRDS("output/PI_chicks_Phi_run.rds")

# Extract estimates of survival from top models
PI_Chick_age_surv <- PI_chicks_Phi_run[[3]]$results$real #top model
PI_Chick_dot_surv <- PI_chicks_Phi_run[[13]]$results$real 
PI_Chick_quadage_surv <- PI_chicks_Phi_run[[23]]$results$real # 2nd model
PI_Chick_hatch_date_surv <- PI_chicks_Phi_run[[8]]$results$real
PI_Chick_mgmt_s_surv <- PI_chicks_Phi_run[[15]]$results$real
PI_Chick_mgmt_l_surv <- PI_chicks_Phi_run[[14]]$results$real
#PI_Chick_mgmt_s_x_quadage_surv <- PI_chicks_Phi_run[[20]]$results$real
#PI_Chick_mgmt_s_quadage_surv <- PI_chicks_Phi_run[[17]]$results$real

Groups <- data.frame(str_split_fixed(rownames(PI_Chick_age_surv), " ", n = 5))
PI_Chick_age_surv <- cbind(Groups, PI_Chick_age_surv)
PI_Chick_age_surv$season <- unlist(substr(PI_Chick_age_surv$X2, 2, 7))
PI_Chick_age_surv$age <- as.numeric(sub("^a", "", PI_Chick_age_surv$X4))
PI_Chick_age_surv$mgmt_s <- unlist(substr(PI_Chick_age_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(PI_Chick_dot_surv), " ", n = 5))
PI_Chick_dot_surv <- cbind(Groups, PI_Chick_dot_surv)
PI_Chick_dot_surv$season <- unlist(substr(PI_Chick_dot_surv$X2, 2, 7))
PI_Chick_dot_surv$age <- as.numeric(sub("^a", "", PI_Chick_dot_surv$X4))
PI_Chick_dot_surv$mgmt_s <- unlist(substr(PI_Chick_dot_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(PI_Chick_quadage_surv), " ", n = 5))
PI_Chick_quadage_surv <- cbind(Groups, PI_Chick_quadage_surv)
PI_Chick_quadage_surv$season <- unlist(substr(PI_Chick_quadage_surv$X2, 2, 7))
PI_Chick_quadage_surv$age <- as.numeric(sub("^a", "", PI_Chick_quadage_surv$X4))
PI_Chick_quadage_surv$mgmt_s <- unlist(substr(PI_Chick_quadage_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(PI_Chick_hatch_date_surv), " ", n = 5))
PI_Chick_hatch_date_surv <- cbind(Groups, PI_Chick_hatch_date_surv)
PI_Chick_hatch_date_surv$season <- unlist(substr(PI_Chick_hatch_date_surv$X2, 2, 7))
PI_Chick_hatch_date_surv$age <- as.numeric(sub("^a", "", PI_Chick_hatch_date_surv$X4))
PI_Chick_hatch_date_surv$mgmt_s <- unlist(substr(PI_Chick_hatch_date_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(PI_Chick_mgmt_s_surv), " ", n = 5))
PI_Chick_mgmt_s_surv <- cbind(Groups, PI_Chick_mgmt_s_surv)
PI_Chick_mgmt_s_surv$season <- unlist(substr(PI_Chick_mgmt_s_surv))
PI_Chick_mgmt_s_surv$age <- as.numeric(sub("^a", "", PI_Chick_mgmt_s_surv$X4))
PI_Chick_mgmt_s_surv$mgmt_s <- unlist(substr(PI_Chick_mgmt_s_surv$X2, 11, 11))

Groups <- data.frame(str_split_fixed(rownames(PI_Chick_mgmt_l_surv), " ", n = 5))
PI_Chick_mgmt_l_surv <- cbind(Groups, PI_Chick_mgmt_l_surv)
PI_Chick_mgmt_l_surv$season <- unlist(substr(PI_Chick_mgmt_l_surv$X2, 2, 7))
PI_Chick_mgmt_l_surv$age <- as.numeric(sub("^a", "", PI_Chick_mgmt_l_surv$X4))
PI_Chick_mgmt_l_surv$mgmt_l <- unlist(substr(PI_Chick_mgmt_l_surv$X2, 10, 10))
...
# constant survival
ggplot() +
  geom_line(data = PI_Chick_dot_surv, aes(x = age, y = estimate, color = as.numeric(season), group = season)) +
  geom_ribbon(data = PI_Chick_dot_surv, aes(x = age, ymin = lcl, ymax = ucl, fill = as.numeric(season), group = season), alpha = 0.2) +
  scale_color_gradient(
    low = "black",
    high = "red",
    name = "season"
...
# quadratic age-dependent survival
ggplot() +
  geom_line(data = filter(PI_Chick_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate)) +
  geom_ribbon(data = filter(PI_Chick_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl), alpha = 0.2) +
  ylab("Daily brood survival (± 95% CI)") +
  xlab("Brood age (days)")
...
ggplot() +
  geom_line(data = filter(PI_Chick_age_surv, X1 == "Phi"), 
            aes(x = age, y = estimate)) +
  geom_ribbon(data = filter(PI_Chick_age_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl), alpha = 0.2) +
  ylab("Daily brood survival (± 95% CI)") +
  xlab("Brood age (days)")
...
# quadratic age-dependent survival and management status
ggplot() +
  geom_line(data = filter(PI_Chick_mgmt_s_quadage_surv, X1 == "Phi"), 
            aes(x = age, y = estimate, color = mgmt_s)) +
  geom_ribbon(data = filter(PI_Chick_mgmt_s_quadage_surv, X1 == "Phi"), 
              aes(x = age, ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  ylab("Daily brood survival (± 95% CI)") +
  xlab("Brood age (days)")
...
all_chicks_ch <- 
  ...
  mutate(mgmt_45 = ifelse(mgmt_l %in% c("4", "5"), "Y", "N") %>% as.factor())
...
# 
# # Create processed RMARK data format as CJS with "Year" as group
# chicks_45.proc <- process.data(chicks_45_ch, model = "CJS",
#                                groups = c("season", "region", 
#                                           "mgmt_l", "mgmt_s"))
# 
# chicks_YN.proc <- process.data(chicks_YN_ch, model = "CJS",
#                                groups = c("season", "region", 
#                                           "mgmt_45", "mgmt_s"))
# 
# # Create the design data
# chicks_45.ddl <- make.design.data(chicks_45.proc)
# chicks_YN.ddl <- make.design.data(chicks_YN.proc)
# 
# # create a quadratic time variable
# time <- c(0:(chicks_45.proc$nocc[1]-1))
# Quadratic <- time^2
# Cubic <- time^3
# quad_time <- data.frame(time, Quadratic, Cubic)
# funday_cov <- data.frame(time = 0:(chicks_45.proc$nocc[1]-1),
#                          funday = chick_ch$funday[1:chicks_45.proc$nocc[1]])
# chicks_45.ddl$p <- merge_design.covariates(chicks_45.ddl$Phi, 
#                                               quad_time, bygroup = FALSE, bytime = TRUE)
# chicks_45.ddl$Phi <- merge_design.covariates(chicks_45.ddl$Phi, 
#                                                 quad_time, bygroup = FALSE, bytime = TRUE)
# chicks_45.ddl$Phi <- merge_design.covariates(chicks_45.ddl$Phi, 
#                                              funday_cov, bygroup = FALSE, bytime = TRUE)
# 
# time <- c(0:(chicks_YN.proc$nocc[1]-1))
# Quadratic <- time^2
# Cubic <- time^3
# quad_time <- data.frame(time, Quadratic, Cubic)
# funday_cov <- data.frame(time = 0:(chicks_YN.proc$nocc[1]-1),
#                          funday = chick_ch$funday[1:chicks_YN.proc$nocc[1]])
# chicks_YN.ddl$p <- merge_design.covariates(chicks_YN.ddl$Phi, 
#                                               quad_time, bygroup = FALSE, bytime = TRUE)
# chicks_YN.ddl$Phi <- merge_design.covariates(chicks_YN.ddl$Phi, 
#                                                 quad_time, bygroup = FALSE, bytime = TRUE)
# chicks_YN.ddl$Phi <- merge_design.covariates(chicks_YN.ddl$Phi, 
#                                              funday_cov, bygroup = FALSE, bytime = TRUE)
```
...
# First assess the best model structure of encounter probability (p) while
# keeping survival (Phi) constant
chicks_p = function(region_, ch_, groups_) 
{
  ch <- 
    ch_ %>% 
    filter(region %in% region_)
  
  ch.proc <- process.data(ch, model = "CJS", groups = groups_)
  ch.ddl <- make.design.data(ch.proc)
  
  time <- c(0:(ch.proc$nocc[1]-1))
  Quadratic <- time^2
  Cubic <- time^3
  
  quad_time <- data.frame(time, Quadratic, Cubic)
  funday_cov <- data.frame(time = 0:(ch.proc$nocc[1]-1),
                           funday = ch$funday[1:ch.proc$nocc[1]])
  ch.ddl$p <- merge_design.covariates(ch.ddl$Phi, 
                                      quad_time, bygroup = FALSE, bytime = TRUE)
  ch.ddl$Phi <- merge_design.covariates(ch.ddl$Phi, 
                                        quad_time, bygroup = FALSE, bytime = TRUE)
  ch.ddl$Phi <- merge_design.covariates(ch.ddl$Phi, 
                                        funday_cov, bygroup = FALSE, bytime = TRUE)
 ...
 # Only include region effects if region_ has >1 unique value
  if (length(unique(ch$region)) > 1) {
    
    # p varies across regions
    p.region <- list(formula = ~region)
    
    # additive effect between region and age
    p.region.age <- list(formula = ~region + Time)
    
    # additive effect between region and quadratic age
    p.region.quadage <- list(formula = ~region + Quadratic)
  }
  ...
  # runs model list in program MARK
  model.list <- mark.wrapper(model.list = cml, data = ch.proc, 
                             ddl = ch.ddl, 
                             threads = 4, brief = TRUE, delete = TRUE)
...
```{r}
# Run model selection to determine best structure of p
MP_chicks_p_run <- chicks_p(region_ = "MP", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

BC_chicks_p_run <- chicks_p(region_ = "BC", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

BSC_chicks_p_run <- chicks_p(region_ = "BSC", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

FP_chicks_p_run <- chicks_p(region_ = "FP", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

PI_chicks_p_run <- chicks_p(region_ = "PI", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

all_chicks_p_run <- chicks_p(region_ = c("PI", "MP", "BC", "BSC", "FP"), 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

# inspect p AIC table to find top model
MP_chicks_p_run # best structure: p(~season + date_j)
BC_chicks_p_run
BSC_chicks_p_run
FP_chicks_p_run
PI_chicks_p_run
...
```{r}
saveRDS(chicks_p_run, "output/chicks_p_run.rds")

#Create AIC model selection table

chicks_p_run <- readRDS(file = "output/chicks_p_run.rds") 

all_chicks_p_run$model.table %>% 
 mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "linear age",
      "season" = "year"))) %>%
  dplyr::select(model, npar, DeltaAICc, weight) %>% gt() %>% 
  cols_label(model = html("<i>All Regions</i>"), 
             npar = "K", DeltaAICc = "DeltaAICc", weight = "weight") %>% 
  fmt_number(columns = DeltaAICc, decimals = 2, use_seps = FALSE) %>% 
  fmt_number(columns = weight, decimals = 3, use_seps = FALSE) %>% 
  cols_align(align = "left", columns = vars(model)) %>% 
  ...
  tab_options(row_group.font.weight = "bold", 
              ...
              table.width = pct(80)) %>%
  tab_header(
    title = "Hooded Plover daily brood resighting probability",
    subtitle = "Relative importance of age, region, and time"
  )

#Create AIC model selection table with delta AIC < 2
chicks_p_run$model.table %>%
   filter(DeltaAICc < 2 | model == "Phi(~1)p(~1)") %>%
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
...
# Extract and wrangle estimates of survival from top model
all_chicks_p_reals_region_quad <- 
  all_chicks_p_run[[6]]$results$real %>%
  # Convert rownames into a separate column
  rownames_to_column(var = "rownames") %>%
  # Split the rownames into 5 separate columns
  separate(rownames, into = paste0("X", 1:5), sep = " ", remove = FALSE) %>%
  filter(X1 == "p") %>%
  # Extract region from X2
  mutate(region = ifelse(nchar(X2) == 12, substr(X2, 8, 9),
                         ifelse(nchar(X2) == 13, substr(X2, 8, 10), "XX")),
         age = as.numeric(str_remove(X4, "^a")))

ggplot() +
  geom_line(data = all_chicks_p_reals_region_quad, 
            aes(x = age, y = estimate)) +
  geom_ribbon(data = all_chicks_p_reals_region_quad, 
              aes(x = age, ymin = lcl, ymax = ucl), 
              alpha = 0.2) +
  scale_color_gradient(
    low = "black",
    high = "red",
...
```{r}
# Find best structure of Phi while using p = ~season * date_j
chicks_Phi = function(region_, ch_, groups_) 
{
  ch <- 
    ch_ %>% 
    filter(region %in% region_)
  ...
  Phi.funday.mgmt_45 <-
    ...
    # Additive effect of funday and management level
  Phi.funday.age <-
    list(formula = ~ funday + Time)

  # Additive effect of funday and management status
  Phi.funday.quadage <-
    list(formula = ~ funday + Quadratic)
  ...
  if (length(unique(ch$region)) > 1) {
    ...
    # additive effect between region and age
    Phi.region.age <- 
      list(formula = ~region + Time)
    
    # additive effect between region and quadratic age
    Phi.region.quadage <- 
      list(formula = ~region + Quadratic)
    
    # additive effect between region and age
    Phi.region.age.funday <- 
      list(formula = ~region + Time + funday)
    
    # additive effect between region and quadratic age
    Phi.region.quadage.funday <- 
      list(formula = ~region + Quadratic + funday)

    # additive effect between region and age
    Phi.region.age.mgmt_l <- 
      list(formula = ~region + Time + mgmt_l)
    
    # additive effect between region and quadratic age
    Phi.region.quadage.mgmt_l <- 
      list(formula = ~region + Quadratic + mgmt_l)
    
        # additive effect between region and age
    Phi.region.age.mgmt_s <- 
      list(formula = ~region + Time + mgmt_s)
    
    # additive effect between region and quadratic age
    Phi.region.quadage.mgmt_s <- 
      list(formula = ~region + Quadratic + mgmt_s)
    
            # additive effect between region and age
    Phi.region.age.mgmt_45 <- 
      list(formula = ~region + Time + mgmt_45)
    
    # additive effect between region and quadratic age
    Phi.region.quadage.mgmt_45 <- 
      list(formula = ~region + Quadratic + mgmt_45)
    
    
    # additive effect between region and quadratic age
    Phi.region.funday <- 
      list(formula = ~region + funday)
    
    # additive effect between region and quadratic age
    Phi.region.date <- 
      list(formula = ~region + date_j)
    
    # Additive effect of funday and management level
    Phi.region.mgmt_l <-
      list(formula = ~ region + mgmt_l)
  
    # Additive effect of funday and management status
    Phi.region.mgmt_s <-
      list(formula = ~ region + mgmt_s)
    
    # Additive effect of funday and management status
    Phi.region.mgmt_45 <-
      list(formula = ~ region + mgmt_45)
    
    # Additive effect of funday and management level
    Phi.region.mgmt_l.funday <-
      list(formula = ~ region + funday + mgmt_l)
  
    # Additive effect of funday and management status
    Phi.region.mgmt_s.funday <-
      list(formula = ~ region + funday + mgmt_s)
    
    # Additive effect of funday and management status
    Phi.region.mgmt_45.funday <-
      list(formula = ~ region + funday + mgmt_45)
    
    # Additive effect of funday and management level
    Phi.region.mgmt_l.date <-
      list(formula = ~ region + date_j + mgmt_l)
  
    # Additive effect of funday and management status
    Phi.region.mgmt_s.date <-
      list(formula = ~ region + date_j + mgmt_s)
    
    # Additive effect of funday and management status
    Phi.region.mgmt_45.date <-
      list(formula = ~ region + date_j + mgmt_45)
  }
...
### region + funday * mgmt_l
```{r}
region_ = c("PI", "MP", "BC", "BSC", "FP")
# region_ = "MP"
groups_ = c("season", "region", "mgmt_l", "mgmt_s", "mgmt_45")
ch_ = all_chicks_ch# %>% filter(mgmt_l %in% c("4", "5"))
{
  ch <- 
    ch_ %>% 
    filter(region %in% region_) %>%
    group_by(region) %>%                     # group by region
    mutate(date_js = (date_j - mean(date_j, na.rm = TRUE)) / 
             sd(date_j, na.rm = TRUE)) %>%
    ungroup()
...
Phi.region.mgmt_l.funday <- 
  mark(data = ch.proc, 
       ddl = ch.ddl, 
       model = "CJS",
       model.parameters = list("Phi" = list(formula = ~region + funday + mgmt_l),
                               "p" = list(formula = ~region + Time)),
       brief = TRUE, 
       delete = TRUE)

Phi.region.mgmt_l.funday$results$beta

# Extract betas and vcv
betas <- Phi.region.mgmt_l.x.funday$results$beta
vcv   <- Phi.region.mgmt_l.x.funday$results$beta.vcv

# Assign proper row/col names to vcv
rownames(vcv) <- rownames(betas)
colnames(vcv) <- rownames(betas)

# Define funday gradient
funday_vals <- seq(min(all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")) %>% pull(funday)),
                   max(all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")) %>% pull(funday)),
                   length.out = 50)
...
# region levels
region_levels <- c("BC","FP","BSC","PI","MP")  # MP as baseline

# number of simulations
n_sims <- 10000

# --- Function to build design matrix with interaction ---
build_design <- function(funday_vals, mgmt_l_level, region_level) {
  n <- length(funday_vals)
  mat <- matrix(0, nrow = n, ncol = length(betas$estimate))
  ...
  # mgmt_l main effect
  if (!is.na(mgmt_l_level) && mgmt_l_level != 0) {
    beta_name <- paste0("Phi:mgmt_l", mgmt_l_level)
    if (beta_name %in% colnames(mat)) mat[, beta_name] <- 1
    
    # interaction with funday
    beta_inter <- paste0("Phi:funday:mgmt_l", mgmt_l_level)
    if (beta_inter %in% colnames(mat)) mat[, beta_inter] <- funday_vals
  }
  
  # region main effect
  if (!is.na(region_level) && region_level != "MP") {
    beta_name <- paste0("Phi:region", region_level)
    if (beta_name %in% colnames(mat)) mat[, beta_name] <- 1
  }
...
# --- Simulate beta draws ---
vcv_pd <- nearPD(vcv)$mat
vcv_pd <- as.matrix(vcv_pd)
set.seed(123)
beta_sims <- mvrnorm(n = n_sims, mu = betas$estimate, Sigma = vcv_pd)

# --- Compute predicted survival for all combinations ---
pred_list <- list()
for (r in region_levels) {
  for (m in mgmt_levels) {
    design_mat <- build_design(funday_vals, m, r)
    eta <- design_mat %*% t(beta_sims)
    Phi_pred <- plogis(eta)
    
    pred_df_temp <- data.frame(
      funday = funday_vals,
      mean   = apply(Phi_pred, 1, mean),
      lcl    = apply(Phi_pred, 1, quantile, 0.025),
      ucl    = apply(Phi_pred, 1, quantile, 0.975),
      mgmt_l = m,
      region = r
    )
    
    pred_list[[paste0(r,"_mgmt",m)]] <- pred_df_temp
  }
}
...
ggplot(pred_df %>% filter(region %in% region_), aes(x = funday, y = mean, color = region, linetype = as.factor(mgmt_l), fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays exposed",
       y = "Apparent daily brood survival (± 95% CI)",
       color = "Region",
       fill  = "Region",
       linetype = "Management level") +
  ...
  facet_grid(.~region)
###
Phi.region.mgmt_l.x.quadage <- 
  mark(data = ch.proc, 
       ddl = ch.ddl, 
       model = "CJS",
       model.parameters = list("Phi" = list(formula = ~ region + Quadratic * mgmt_l),
                               "p" = list(formula = ~region + Quadratic)),
       brief = TRUE, 
       delete = TRUE)

Phi.region.mgmt_l.x.quadage$results$beta

# Extract and wrangle estimates of survival from top model
all_chicks_Phi.region.mgmt_l.quadage_reals <- 
  Phi.region.mgmt_l.quadage$results$real %>%
  # Convert rownames into a separate column
  rownames_to_column(var = "rownames") %>%
  # Split the rownames into 5 separate columns
  separate(rownames, into = paste0("X", 1:5), sep = " ", remove = FALSE) %>%
  ...
  mutate(region = ifelse(nchar(X2) == 12, substr(X2, 8, 9),
                         ifelse(nchar(X2) == 13, substr(X2, 8, 10), "XX")),
         mgmt_l = ifelse(nchar(X2) == 12, substr(X2, 10, 10),
                         ifelse(nchar(X2) == 13, substr(X2, 11, 11), "XX")),
         age = as.numeric(str_remove(X4, "^a")))

cols <- brewer.pal(5, "Dark2")  # for 5 regions
ggplot(all_chicks_Phi.region.mgmt_l.quadage_reals, 
       aes(x = age, y = estimate, color = region, linetype = as.factor(mgmt_l), fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays exposed",
       y = "Apparent daily brood survival (± 95% CI)",
...
Phi.region.mgmt_l.funday.quadage <- 
  mark(data = ch.proc, 
       ddl = ch.ddl, 
       model = "CJS",
       model.parameters = list("Phi" = list(formula = ~region + funday + mgmt_l + Quadratic),
                               "p" = list(formula = ~region + Quadratic)),
       brief = TRUE, 
       delete = TRUE)

Phi.region.mgmt_l.funday.quadage$results$beta

 # ---- Extract betas and vcv ----#
betas <- Phi.region.mgmt_l.funday.quadage$results$beta
vcv   <- Phi.region.mgmt_l.funday.quadage$results$beta.vcv

# # keep only Phi parameters (drop detection ones)
# betas <- betas[grep("^Phi:", rownames(betas)), , drop = FALSE]
# vcv   <- vcv[rownames(betas), rownames(betas)]
...
# ---- funday gradient ----
funday_vals <- seq(
  min(all_chicks_ch %>% filter(mgmt_l %in% c("1","2","3","4","5","6")) %>% pull(funday), na.rm = TRUE),
  max(all_chicks_ch %>% filter(mgmt_l %in% c("1","2","3","4","5","6")) %>% pull(funday), na.rm = TRUE),
  length.out = 50
)

# ---- mgmt and region levels ----
mgmt_beta_names <- grep("^Phi:mgmt_l", rownames(betas), value = TRUE)
mgmt_levels <- sub("^Phi:mgmt_l", "", mgmt_beta_names) %>% as.integer()
mgmt_levels_all <- sort(unique(c(0, mgmt_levels)))

region_levels <- c("BC","FP","BSC","PI","MP")  # MP = baseline

# ---- sims ----
n_sims <- 10000
set.seed(123)
vcv_pd <- as.matrix(nearPD(vcv)$mat)
beta_sims <- mvrnorm(n = n_sims, mu = betas$estimate, Sigma = vcv_pd)
...
build_design <- function(funday_vals, mgmt_l_level, region_level) {
  ...
  # Intercept
  if ("Phi:(Intercept)" %in% colnames(mat)) mat[, "Phi:(Intercept)"] <- 1
  
  # funday linear term
  if ("Phi:funday" %in% colnames(mat)) mat[, "Phi:funday"] <- funday_vals
  
  # funday quadratic term
  if ("Phi:Quadratic" %in% colnames(mat)) mat[, "Phi:Quadratic"] <- funday_vals^2
  ...
  # region main effects (baseline MP)
  if (!is.na(region_level) && region_level != "MP") {
    beta_name <- paste0("Phi:region", region_level)
    if (beta_name %in% colnames(mat)) mat[, beta_name] <- 1
  }
...
for (r in region_levels) {
  for (m in mgmt_levels_all) {
    ...
    pred_df_temp <- data.frame(
      funday = funday_vals,
      mean   = rowMeans(Phi_pred),
      lcl    = apply(Phi_pred, 1, quantile, probs = 0.025),
      ucl    = apply(Phi_pred, 1, quantile, probs = 0.975),
      mgmt_l = m,
      region = r
    )
...
ggplot(pred_df,
       aes(x = funday, y = mean, color = region, linetype = as.factor(mgmt_l), fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays exposed",
       y = "Apparent daily brood survival (± 95% CI)",
...
### region + funday * mgmt_l + quadratic
```{r}
region_ = c("PI", "MP", "BC", "BSC", "FP")
# region_ = "MP"
groups_ = c("season", "region", "mgmt_l", "mgmt_s", "mgmt_45")
...
Phi.region.mgmt_l.Time <- 
  mark(data = ch.proc, 
       ddl = ch.ddl, 
       model = "CJS",
       model.parameters = list("Phi" = list(formula = ~region + Time + mgmt_l),
                               "p" = list(formula = ~region + Time)),
       brief = TRUE, 
       delete = TRUE)

Phi.region.mgmt_l.Time$results$beta

# --- Extract betas and vcv ---
betas <- Phi.region.mgmt_l.Time$results$beta
vcv   <- Phi.region.mgmt_l.Time$results$beta.vcv

# Assign proper row/col names to vcv
rownames(vcv) <- rownames(betas)
colnames(vcv) <- rownames(betas)

# --- Define Time gradient ---
Time_vals <- seq(
  1, 35
)

# --- mgmt_l levels and region levels ---
mgmt_levels <- c(4, 5)
region_levels <- c("BC", "FP", "BSC", "PI", "MP")  # MP as baseline
...
# --- Function to build design matrix (no quadratic, no interaction) ---
build_design <- function(Time_vals, mgmt_l_level, region_level) {
  n <- length(Time_vals)
  mat <- matrix(0, nrow = n, ncol = length(betas$estimate))
  colnames(mat) <- rownames(betas)
  ...
  # Time linear term
  if ("Phi:Time" %in% colnames(mat)) {
    mat[, "Phi:Time"] <- Time_vals
  }
  
  # mgmt_l main effect
  beta_name <- paste0("Phi:mgmt_l", mgmt_l_level)
  if (beta_name %in% colnames(mat)) {
    mat[, beta_name] <- 1
  }
...
vcv_pd <- as.matrix(vcv_pd)
set.seed(123)
beta_sims <- MASS::mvrnorm(n = n_sims, mu = betas$estimate, Sigma = vcv_pd)

# --- Compute predicted survival for all combinations ---
pred_list <- list()
for (r in region_levels) {
  for (m in mgmt_levels) {
    design_mat <- build_design(Time_vals, m, r)
    eta <- design_mat %*% t(beta_sims)
    Phi_pred <- plogis(eta)
...
ggplot(pred_df %>% filter(region %in% region_levels), 
       aes(x = Time, y = mean, color = region, linetype = mgmt_l, fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.5, color = NA) +
  labs(
    x = "Brood Age (days)",
    y = "Apparent daily brood survival (±95% CI)",
    linetype = "Management"
  ) +
  scale_color_manual(values = cols, guide = "none") +   # suppress region color legend
  scale_fill_manual(values = cols, guide = "none") +    # suppress region fill legend
  scale_linetype_manual(
    values = c("4" = "solid", "5" = "twodash"),          # optional: make lines visually distinct
    labels = c("4" = "fencing", "5" = "wardens")        # rename legend entries
  ) +
  theme_minimal() +
...
ggplot(all_chicks_p_reals_region_quad, 
       aes(x = age, y = estimate, color = region, fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.5, color = NA) +
  labs(
    x = "Brood Age (days)",
    y = "Daily brood detection probability (±95% CI)",
  ) +
  scale_color_manual(values = cols, guide = "none") +   # suppress region color legend
  scale_fill_manual(values = cols, guide = "none") +    # suppress region fill legend
  theme_minimal() +
...
```{r}
# Run model selection to determine best structure of p
MP_chicks_Phi_run <- chicks_Phi(region_ = "MP", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

BC_chicks_Phi_run <- chicks_Phi(region_ = "BC", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

BSC_chicks_Phi_run <- chicks_Phi(region_ = "BSC", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

FP_chicks_Phi_run <- chicks_Phi(region_ = "FP", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

PI_chicks_Phi_run <- chicks_Phi(region_ = "PI", 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

all_chicks_Phi_run <- chicks_Phi(region_ = c("PI", "MP", "BC", "BSC", "FP"), 
                            ch_ = all_chicks_ch,
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

# inspect p AIC table to find top model
MP_chicks_Phi_run # best structure: p(~season + date_j)
BC_chicks_Phi_run
BSC_chicks_Phi_run
FP_chicks_Phi_run
PI_chicks_Phi_run
...
# Run model selection to determine best structure of p
MP_chicks_45_Phi_run <- chicks_Phi(region_ = "MP", 
                            ch_ = all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")),
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

BC_chicks_45_Phi_run <- chicks_Phi(region_ = "BC", 
                            ch_ = all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")),
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

BSC_chicks_45_Phi_run <- chicks_Phi(region_ = "BSC", 
                            ch_ = all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")),
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

FP_chicks_45_Phi_run <- chicks_Phi(region_ = "FP", 
                            ch_ = all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")),
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

PI_chicks_45_Phi_run <- chicks_Phi(region_ = "PI", 
                            ch_ = all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")),
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

all_chicks_45_Phi_run <- chicks_Phi(region_ = c("PI", "MP", "BC", "BSC", "FP"), 
                            ch_ = all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")),
                            groups = c("season", "region", 
                                       "mgmt_l", "mgmt_s", "mgmt_45"))

# inspect p AIC table to find top model
MP_chicks_45_Phi_run # best structure: p(~season + date_j)
BC_chicks_45_Phi_run
BSC_chicks_45_Phi_run
FP_chicks_45_Phi_run
PI_chicks_45_Phi_run
all_chicks_45_Phi_run


all_chicks_45_Phi_run$model.table %>%
   filter(DeltaAICc < 20 | model == "Phi(~1)p(~region + Time)") %>% 
 mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       "date_j" = "hatch date",
      "Time" = "linear age",
  ...
  tab_style(style = cell_text(weight = "bold"),
    locations = cells_body(
      columns = vars(model),
      rows = model == "Phi(~1)p(~region + linear age)")) %>% 
   text_transform(
    locations = cells_body(columns = vars(model)),
    fn = function(x) gsub("\\)", ") ", x) ) %>%
  tab_options(row_group.font.weight = "bold", 
              ...
              table.width = pct(100)) %>%
  tab_header(
    title = "Hooded Plover daily brood apparent survival",
    subtitle = "Relative importance of age, region, and time"
  )
```
...
# Extract betas and vcv
betas <- all_chicks_45_Phi_run[[31]]$results$beta
vcv   <- all_chicks_45_Phi_run[[31]]$results$beta.vcv
...
# Define funday gradient
funday_vals <- 
  seq(all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")) %>% pull(funday) %>% min(), 
      all_chicks_ch %>% filter(mgmt_l %in% c("4", "5")) %>% pull(funday) %>% max(), 
      length.out = 50)

# mgmt_l levels (adjust depending on your data coding)
mgmt_levels <- c(4,5)  # you had 4 & 5 in your previous plot

# region levels
region_levels <- c("BC","FP","BSC","PI","MP")  # adjust if MP is baseline

# number of simulations
n_sims <- 10000

# --- Function to build design matrix for given funday, mgmt_l, and region ---
build_design <- function(funday_vals, mgmt_l_level, region_level) {
  n <- length(funday_vals)
  mat <- matrix(0, nrow = n, ncol = length(betas$estimate))
  colnames(mat) <- rownames(betas)
  ...
  # region
  if (!is.na(region_level) && region_level != "MP") {  # assuming MP is baseline
    beta_name <- paste0("Phi:region", region_level)
    if (beta_name %in% colnames(mat)) mat[, beta_name] <- 1
  }
...
# --- Simulate beta draws ---
vcv_pd <- nearPD(vcv)$mat  # returns a "Matrix" object
vcv_pd <- as.matrix(vcv_pd)  # convert back to normal matrix
set.seed(123)
beta_sims <- mvrnorm(n = n_sims, mu = betas$estimate, Sigma = vcv_pd)
...
# --- Plot ---
cols <- brewer.pal(5, "Dark2")  # for 5 regions
ggplot(pred_df, aes(x = funday, y = mean, color = region, linetype = as.factor(mgmt_l), fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays exposed",
       y = "Apparent daily brood survival (± 95% CI)",
...
# Extract betas and VCV
betas <- all_chicks_Phi_run[[30]]$results$beta
vcv   <- all_chicks_Phi_run[[30]]$results$beta.vcv

# Fix near-singular VCV
vcv_pd <- nearPD(vcv)$mat
vcv_pd <- as.matrix(vcv_pd)

# Define covariate values
date_vals <- 
  seq(all_chicks_ch %>% pull(date_j) %>% min(), 
      all_chicks_ch %>% pull(date_j) %>% max(), 
      length.out = 50)
mgmt_levels <- c(0, 1, 2, 3, 4, 5, 6)  # adjust if needed
regions <- c("MP", "BC", "FP", "BSC", "PI")  # adjust names if needed
...
# --- Function to build design matrix for Phi ---
build_design_Phi <- function(region, date_val, mgmt_l) {
  n <- length(date_val)
  mat <- matrix(0, nrow = n, ncol = length(betas$estimate))
  colnames(mat) <- rownames(betas)
  ...
  # Region
  if (region != "MP") mat[, paste0("Phi:region", region)] <- 1
  
  # Date
  mat[, "Phi:date_j"] <- date_val
  
  # mgmt_l
  if (mgmt_l != 0) mat[, paste0("Phi:mgmt_l", mgmt_l)] <- 1
...
# Example: loop over regions, mgmt levels, and funday values for Phi
for (region in regions) {
  for (mgmt in mgmt_levels) {
    
    design_mat <- build_design_Phi(region, date_vals, mgmt)
    ...
    pred_list[[paste(region, mgmt, sep = "_")]] <- data.frame(
      date_j = date_vals,
      mean   = Phi_mean,
      lcl    = Phi_lcl,
      ucl    = Phi_ucl,
...
# --- Plot ---
cols <- brewer.pal(7, "Dark2")  # one color per region

ggplot(pred_df, aes(x = date_j, y = mean, color = as.factor(mgmt_l), fill = as.factor(mgmt_l))) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  scale_color_manual(values = cols) +
  scale_fill_manual(values = cols) +
  labs(x = "Date (j)", y = "Apparent daily brood survival (± 95% CI)",
...
```{r}
# Extract and wrangle estimates of survival from top model
all_chicks_Phi_reals_region_age_mgmt_l <- 
  all_chicks_45_Phi_run[[39]]$results$real %>%
  # Convert rownames into a separate column
  rownames_to_column(var = "rownames") %>%
  # Split the rownames into 5 separate columns
  separate(rownames, into = paste0("X", 1:5), sep = " ", remove = FALSE) %>%
...
cols <- brewer.pal(5, "Dark2")  # for 5 regions
ggplot(all_chicks_Phi_reals_region_age_mgmt_l, 
       aes(x = age, y = estimate, color = region, linetype = as.factor(mgmt_l), fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays exposed",
...
# extract the beta and vcv estimates
funday_mgmt_l_vcv <- 
  all_chicks_45_Phi_run[[27]]$results$beta.vcv

funday_mgmt_l_betas <- 
  all_chicks_45_Phi_run[[27]]$results$beta
...
```{r}
# Run model selection to determine best structure of Phi
chicks_Phi_run <- chicks_Phi()

# inspect Phi AIC table to find top model
chicks_Phi_run # best structure: Phi(~Year * Day_of_Season)

saveRDS(chicks_Phi_run, "output/chicks_Phi_run.rds")

#Create AIC model selection table

chicks_Phi_run <- readRDS(file = "output/chicks_Phi_run.rds") 

chicks_Phi_run$model.table %>% 
 mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
      "mgmt_l" = "management level",
      "mgmt_s" = "management status",
...
### Extract results and visualize Phi
```{r}
chicks_Phi_run <- readRDS("output/chicks_Phi_run.rds")

Chick_mgmt_l_region_surv <- chicks_Phi_run[[17]]$results$real

Groups <- data.frame(str_split_fixed(rownames(Chick_mgmt_l_region_surv), " ", n = 5))
Chick_mgmt_l_region_surv <- cbind(Groups, Chick_mgmt_l_region_surv)
Chick_mgmt_l_region_surv <- 
Chick_mgmt_l_region_surv %>% 
  filter(X1 == "Phi") %>% 
  mutate(region = ifelse(nchar(X2) == 11, substr(Chick_mgmt_l_region_surv$X2, 8, 9),
                         ifelse(nchar(X2) == 12, substr(Chick_mgmt_l_region_surv$X2, 8, 10), "XX")),
         mgmt_l = ifelse(nchar(X2) == 11, substr(Chick_mgmt_l_region_surv$X2, 10, 10),
                         ifelse(nchar(X2) == 12, substr(Chick_mgmt_l_region_surv$X2, 11, 11), "XX")))


mgmt_level_plot_chick_surv <- 
  ggplot() +
  geom_line(data = Chick_mgmt_l_region_surv, 
            aes(x = mgmt_l, y = estimate^35, color = region, group = region),
            position = position_dodge(width = 0.5), alpha = 0.5, size = 1) +
  geom_errorbar(data = Chick_mgmt_l_region_surv, 
                aes(ymin = lcl^35, ymax = ucl^35, 
                    x = mgmt_l, 
                    y = estimate, group = region), position = position_dodge(width = 0.5),
                alpha = 0.5, color = "black", width = 0.3, lwd = 0.5) +
  geom_point(data = Chick_mgmt_l_region_surv, 
             aes(x = mgmt_l, y = estimate^35, fill = region),
             shape = 21, size = 4, position = position_dodge(width = 0.5)) +
  scale_colour_brewer(palette = "Set1") +
  luke_theme +
  ...
  ylab("Apparent fledgling success (± 95% CI)") +
  scale_x_discrete(
    breaks = c("4", "5"),
    labels = c("fencing", "wardens")
  ) +
  scale_colour_brewer(palette = "Dark2", 
                      name = "Region", 
labels = c("BSC" = "Bellarine-Surf Coast", 
...
ggsave(plot = mgmt_level_plot_chick_surv, 
       filename = "mgmt_level_plot_chick_surv.png", 
       path = "results/", height = 6, width = 6)

### plot the survival trend of the Phi(~funday + mgmt_l)p(~Quadratic) model ----
# extract the beta and vcv estimates
quadage_mgmt_l_vcv <- 
  chicks_Phi_run[[20]]$results$beta.vcv

quadage_mgmt_l_betas <- 
  chicks_Phi_run[[20]]$results$beta

# Assign the correct names to the vcv matrix
rownames(quadage_mgmt_l_vcv) <- rownames(quadage_mgmt_l_betas)
colnames(quadage_mgmt_l_vcv) <- rownames(quadage_mgmt_l_betas)

# Define funday gradient
quadage_vals <- 
  seq(min(chicks.ddl$Phi$Quadratic), max(chicks.ddl$Phi$Quadratic), 
      length.out = 50)

# mgmt_l levels (assuming 0-5 coded, adjust if different)
mgmt_levels <- paste0("mgmt_l", 0:6)  # adjust 0 if intercept represents mgmt_l0
region_levels <- c("MP", "BC", "FP", "BSC", "PI")
# Number of simulations
n_sims <- 10000

# --- Function to build design matrix for a given mgmt_l ---
build_design <- function(quadage_vals, region_level) {
  n <- length(quadage_vals)
  
  # Create an empty matrix with same columns as betas
  mat <- matrix(0, nrow = n, ncol = length(quadage_mgmt_l_betas$estimate))
  colnames(mat) <- rownames(quadage_mgmt_l_betas)
  
  # Fill intercept
  mat[, "Phi:(Intercept)"] <- 1
  
  # Fill funday covariate
  mat[, "Phi:Quadratic"] <- quadage_vals
  
  # Fill the mgmt_l dummy
  if (region_level != "MP") {   # baseline is 0, others add their beta
    mat[, paste0("Phi:region", region_level)] <- 1
  }
...
# --- Simulate beta draws ---
set.seed(123)
beta_sims <- mvrnorm(n = n_sims, mu = quadage_mgmt_l_betas$estimate, Sigma = quadage_mgmt_l_vcv)

# --- Compute predicted survival and 95% CIs ---
pred_list <- list()

for (i in c("BC", "FP", "BSC", "PI", "MP")) {  # mgmt_l levels
  design_mat <- build_design(quadage_vals, i)
  ...
  pred_list[[paste0("region", i)]] <- data.frame(
    quadage = quadage_vals,
    mean   = Phi_mean,
    lcl    = Phi_lcl,
    ucl    = Phi_ucl,
    region = i
...
chick_ch %>% pull(mgmt_l) %>% table()
# note that I removed the prediction of the management level 2 because it failed to properly estimate this likely due to low sample size (only 4 broods experienced this treatment)

quadage_region_brood_surv_plot <- 
  ggplot(pred_df, #%>% 
         # filter(mgmt_l != 2), 
       aes(x = sqrt(quadage), y = mean, color = region, fill = region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), alpha = 0.2, color = NA) +
  labs(x = "Number of fundays (weekend days and holidays) exposed to", 
       y = "Apparent daily brood survival (± 95% CI)",  color = "Management\nlevel",  # line break with \n
...
# extract the beta and vcv estimates
funday_mgmt_l_vcv <- 
  chicks_Phi_run[[7]]$results$beta.vcv

funday_mgmt_l_betas <- 
  chicks_Phi_run[[7]]$results$beta
...
# mgmt_l levels (assuming 0-5 coded, adjust if different)
mgmt_levels <- paste0("mgmt_l", 4:5)  # adjust 0 if intercept represents mgmt_l0
...
build_design <- function(funday_vals, mgmt_l_level) {
  ...
  # Fill the mgmt_l dummy
  if (mgmt_l_level != 4) {   # baseline is 0, others add their beta
    mat[, paste0("Phi:mgmt_l", mgmt_l_level)] <- 1
  }
...
beta_sims <- 
  mvrnorm(n = n_sims, 
          mu = funday_mgmt_l_betas$estimate, 
          Sigma = funday_mgmt_l_vcv)
...
# PI_Chick_mgmt_l_surv <- PI_chicks_Phi_run[[14]]$results$real
#PI_Chick_mgmt_s_x_quadage_surv <- PI_chicks_Phi_run[[20]]$results$real
Chick_mgmt_l_quadage_surv <- chicks_Phi_run[[4]]$results$real

# Groups <- data.frame(str_split_fixed(rownames(Chick_mgmt_l_quadage_surv), " ", n = 5))
# Chick_mgmt_l_quadage_surv <- cbind(Groups, Chick_mgmt_l_quadage_surv)
# Chick_mgmt_l_quadage_surv$season <- unlist(substr(Chick_mgmt_l_quadage_surv$X2, 2, 7))
# Chick_mgmt_l_quadage_surv$age <- as.numeric(sub("^a", "", Chick_mgmt_l_quadage_surv$X4))
# Chick_mgmt_l_quadage_surv$mgmt_s <- unlist(substr(Chick_mgmt_l_quadage_surv$X2, 11, 11))
...
## results table
humans_fun_day__fits_table <- humans_fun_day__fits %>%
  mutate(region = dplyr::recode(region, !!!region_names))

humans_fun_day__fits_table %>%
  mutate(
    funday = str_to_title(funday),
    `Estimate (95% CI)` = sprintf("%.3f (<i>%.3f, %.3f</i>)", fit, lower, upper)) %>%
  select(region, funday, `Estimate (95% CI)`, se) %>%  # keep only desired columns in order
  arrange(region, funday) %>%
  gt(groupname_col = "region") %>%
  tab_header(
    title = md("**Fundays and people**")) %>%
  cols_label(
     funday = "Funday",
    `Estimate (95% CI)` = "Estimate (95% CI)",
    se = "Standard Error") %>%
  fmt_number(columns = vars(se),
    decimals = 3,
    use_seps = FALSE) %>%
  fmt_markdown(columns = vars(`Estimate (95% CI)`)) %>%
  cols_align(align = "left",
    columns = vars(funday)) %>%
  cols_align(align = "center",
    columns = vars(`Estimate (95% CI)`, se)) %>%
...
## results table
dogs_fun_day__fits_table <- dogs_fun_day__fits %>%
  mutate(region = dplyr::recode(region, !!!region_names))

dogs_fun_day__fits_table %>%
   mutate(
    funday = str_to_title(funday),
    `Estimate (95% CI)` = sprintf("%.3f (<i>%.3f, %.3f</i>)", fit, lower, upper)) %>%
  select(region, funday, `Estimate (95% CI)`, se) %>%  # keep only desired columns in order
...
## results table
dogs_off_fun_day__fits_table <- dogs_off_fun_day__fits %>%
  mutate(region = dplyr::recode(region, !!!region_names))

dogs_off_fun_day__fits_table %>%
  mutate(
    funday = str_to_title(funday),
    `Estimate (95% CI)` = sprintf("%.3f (<i>%.3f, %.3f</i>)", fit, lower, upper)) %>%
  select(region, funday, `Estimate (95% CI)`, se) %>%  # keep only desired columns in order
  ...
  gt(groupname_col = "region") %>%
  tab_header(
    title = md("**Fundays and off-leash dogs**")) %>%
  cols_label(
     funday = "Funday",
    `Estimate (95% CI)` = "Estimate (95% CI)",
    se = "Standard Error") %>%
...
## results table
pred_birds_fun_day__fits_table <- pred_birds_fun_day__fits %>%
  mutate(region = dplyr::recode(region, !!!region_names))

pred_birds_fun_day__fits_table %>%
  mutate(
    funday = str_to_title(funday),
    `Estimate (95% CI)` = sprintf("%.3f (<i>%.3f, %.3f</i>)", fit, lower, upper)) %>%
  select(region, funday, `Estimate (95% CI)`, se) %>%  # keep only desired columns in order
  ...
  gt(groupname_col = "region") %>%
  tab_header(
    title = md("**Fundays and predatory birds**")) %>%
  cols_label(
     funday = "Funday",
    `Estimate (95% CI)` = "Estimate (95% CI)",
    se = "Standard Error") %>%
...
Phillip Island
```{r}
tbl_regression(gulls_fun_day_PI_, intercept = TRUE, 
               label = list(obs_date2 ~ "Date", funday ~ "Weekends & Holidays"))
```
...
## results table
gulls_fun_day__fits_table <- gulls_fun_day__fits %>%
  mutate(region = dplyr::recode(region, !!!region_names))

gulls_fun_day__fits_table %>%
 mutate(
    funday = str_to_title(funday),
    `Estimate (95% CI)` = sprintf("%.3f (<i>%.3f, %.3f</i>)", fit, lower, upper)) %>%
  select(region, funday, `Estimate (95% CI)`, se) %>%  # keep only desired columns in order
