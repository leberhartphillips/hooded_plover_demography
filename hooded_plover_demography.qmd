...
MP_nest_ID <-
  suppressMessages(
    ...
    read_excel(paste0("data/", "MP Nesting Summary All Years_19062025.xlsx"),
               sheet = paste0("MP", " ", "2012", "_", str_sub("2013", 3, 4)),
               col_types = "text", na = "n/a")))%>% 
  select(`Nest ID`, `Chick manged?`, `Chick management type`) %>%
...
MP_chick_data_clean <- MP_chick_data_clean %>%
  left_join(
    funday_summary_MP %>% select(nest_ID, funday_total),
    by = "nest_ID",
...
MP_Chick_funday_mgmt_s_surv  <- MP_chicks_Phi_run[[15]]$results$real
MP_Chick_funday_mgmt_l_surv  <- MP_chicks_Phi_run[[14]]$results$real # TOP MODEL (and most convincing)
