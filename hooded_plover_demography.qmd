...
MP_chick_data <-
  suppressMessages(
   ...
   left_join(., MP_nest_ID %>% 
                select(nest_ID, `Chick manged?`,`Chick management type`), by = "nest_ID") %>% 
rename(management_type = `Chick management type`,
  management_status = `Chick manged?`)
...
MP_chick_data_clean_wide <- 
  ...
  pivot_wider(
    id_cols = c(nest_ID, management_level_max, management_status_max, season, region, hatch_date, funday_total),
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data <- BC_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
BC_chick_data_clean_wide <- BC_chick_data_clean_wide %>%
  select(nest_ID, management_level_max, management_status_max, season, region, hatch_date, funday_total, all_of(age_cols))
...
#### wrangle management data ----
 ...
 # dplyr::select(-other, -sign_nest_no_sign_access, -fence_no_sign, 
  ...
  mutate(management_status = ifelse(management_level == 0 & !is.na(management_level), "N", 
                                    ifelse(management_level > 0 & !is.na(management_level), "Y", 
                                           NA))) %>% 
  mutate(management_status = ifelse(management_level == 1, "N", management_status)) %>% 
  group_by(season) %>%
  mutate(season = as.factor(season),
         habitat = as.factor(habitat),
         management_status = as.factor(management_status)) %>% 
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data <- FP_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
# calculate hatch dates from the chick data (first observation and developmental stage)
hatch_dates_from_chick_data <- PI_chick_data %>%
  mutate(date = dmy(date)) %>%    # convert from "DDMMYYYY" to Date
  group_by(nest_ID) %>%
  slice_min(order_by = date, n = 1, with_ties = FALSE) %>%
  mutate(hatch_date2 = date + age_offsets[chick_age]) %>%
...
#BSC - completed, still to fix other regions
#Total number of broods
nrow(BSC_chick_ch)
# Number of broods per management status
BSC_chick_ch %>%
...
# Extract estimates of survival from top models
BC_Chick_dot_surv <- BC_chicks_Phi_run[[11]]$results$real
BC_Chick_hatch_date_x_mgmt_s_surv <- BC_chicks_Phi_run[[11]]$results$real
BC_Chick_quadage_surv <- BC_chicks_Phi_run[[18]]$results$real
...
BSC_Chick_mgmt_l_quadage_surv <- BSC_chicks_Phi_run[[22]]$results$real # Top model - there seems to be a bunch of nest with DNS = 1 for certain groups. Doesn't seem right
