...
```{r}
##MP
# Jas and Mike attempt to calculate fundays
...
funday_summary_MP <- MP_chick_data_clean %>%
  rowwise() %>%
   mutate(
    ...
    # Total fundays = weekends + holidays
     funday_total = weekend_days + holiday_days
  ) %>%
  ungroup()
...
Chick_quadage_surv <- MP_chicks_Phi_run[[23]]$results$real
Chick_hatch_date_surv <- MP_chicks_Phi_run[[8]]$results$real
Chick_mgmt_s_surv <- MP_chicks_Phi_run[[20]]$results$real
Chick_mgmt_l_surv <- MP_chicks_Phi_run[[19]]$results$real
Chick_mgmt_s_x_quadage_surv <- MP_chicks_Phi_run[[25]]$results$real
Chick_mgmt_s_quadage_surv <- MP_chicks_Phi_run[[22]]$results$real
Chick_mgmt_l_quadage_surv <- MP_chicks_Phi_run[[21]]$results$real
Chick_funday_x_mgmt_s_surv      <- MP_chicks_Phi_run[[18]]$results$real
Chick_funday_x_mgmt_l_surv      <- MP_chicks_Phi_run[[14]]$results$real
Chick_funday_mgmt_s_surv  <- MP_chicks_Phi_run[[15]]$results$real
Chick_funday_surv            <- MP_chicks_Phi_run[[16]]$results$real
...
Groups <- data.frame(str_split_fixed(rownames(Chick_funday_x_mgmt_s_surv), " ", n = 5))
Chick_funday_x_mgmt_s_surv <- cbind(Groups, Chick_funday_x_mgmt_s_surv)
Chick_funday_x_mgmt_s_surv$season <- unlist(substr(Chick_funday_x_mgmt_s_surv$X2, 2, 7))
Chick_funday_x_mgmt_s_surv$age <- as.numeric(sub("^a", "", Chick_funday_x_mgmt_s_surv$X4))
Chick_funday_x_mgmt_s_surv$mgmt_l <- unlist(substr(Chick_funday_x_mgmt_s_surv$X2, 10, 10))

Groups <- data.frame(str_split_fixed(rownames(Chick_funday_x_mgmt_l_surv), " ", n = 5))
Chick_funday_x_mgmt_l_surv <- cbind(Groups, Chick_funday_x_mgmt_l_surv)
Chick_funday_x_mgmt_l_surv$season <- unlist(substr(Chick_funday_x_mgmt_l_surv$X2, 2, 7))
Chick_funday_x_mgmt_l_surv$age <- as.numeric(sub("^a", "", Chick_funday_x_mgmt_l_surv$X4))
Chick_funday_x_mgmt_l_surv$mgmt_l <- unlist(substr(Chick_funday_x_mgmt_l_surv$X2, 10, 10))

Groups <- data.frame(str_split_fixed(rownames(Chick_funday_mgmt_s_surv), " ", n = 5))
Chick_funday_mgmt_s_surv <- cbind(Groups, Chick_funday_mgmt_s_surv)
Chick_funday_mgmt_s_surv$season <- unlist(substr(Chick_funday_mgmt_s_surv$X2, 2, 7))
Chick_funday_mgmt_s_surv$age <- as.numeric(sub("^a", "", Chick_funday_mgmt_s_surv$X4))
Chick_funday_mgmt_s_surv$mgmt_l <- unlist(substr(Chick_funday_mgmt_s_surv$X2, 10, 10))

Groups <- data.frame(str_split_fixed(rownames(Chick_funday_surv), " ", n = 5))
Chick_funday_surv <- cbind(Groups, Chick_funday_surv)
Chick_funday_surv$season <- unlist(substr(Chick_funday_surv$X2, 2, 7))
Chick_funday_surv$age <- as.numeric(sub("^a", "", Chick_funday_surv$X4))
Chick_funday_surv$mgmt_l <- unlist(substr(Chick_funday_surv$X2, 10, 10))
...
# --- Model 18: Phi(~funday * mgmt_s) ---
ggplot(Chick_funday_x_mgmt_s_surv %>% filter(X1 == "Phi"), 
       aes(x = funday, y = estimate, color = mgmt_s, group = mgmt_s)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  labs(
    title = "Model 18: Phi(~funday * mgmt_s)",
    x = "Fundays (weekends/holidays)",
    y = "Daily brood survival (Phi)",
    color = "Small-scale management",
    fill = "Small-scale management"
  ) +
  scale_y_continuous(limits = c(0,1)) +
  theme_minimal(base_size = 14)

# --- Model 14: Phi(~funday + mgmt_l) ---
ggplot(Chick_funday_x_mgmt_l_surv %>% filter(X1 == "Phi"), 
       aes(x = funday, y = estimate, color = mgmt_l, group = mgmt_l)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl, fill = mgmt_l), alpha = 0.2) +
  labs(
    title = "Model 14: Phi(~funday + mgmt_l)",
    x = "Fundays (weekends/holidays)",
    y = "Daily brood survival (Phi)",
    color = "Large-scale management",
    fill = "Large-scale management"
  ) +
  scale_y_continuous(limits = c(0,1)) +
  theme_minimal(base_size = 14)

# --- Model 15: Phi(~funday + mgmt_s) ---
ggplot(Chick_funday_mgmt_s_surv %>% filter(X1 == "Phi"), 
       aes(x = funday, y = estimate, color = mgmt_s, group = mgmt_s)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = lcl, ymax = ucl, fill = mgmt_s), alpha = 0.2) +
  labs(
    title = "Model 15: Phi(~funday + mgmt_s)",
    x = "Fundays (weekends/holidays)",
    y = "Daily brood survival (Phi)",
    color = "Small-scale management",
    fill = "Small-scale management"
...
# --- Model 16: Phi(~funday) ---
ggplot(Chick_funday_surv %>% filter(X1 == "Phi"), 
       aes(x = funday, y = estimate)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lcl, ymax = ucl), width = 0.1) +
  labs(
    title = "Model 16: Phi(~funday)",
    x = "Fundays (weekends/holidays)",
    y = "Daily brood survival (Phi)"
  ) +
  scale_y_continuous(limits = c(0,1)) +
...
# inspect Phi AIC table to find top model
BC_chicks_Phi_run # best structure: Phi(~Year * Day_of_Season)
...
# Extract estimates of survival from top models
Chick_dot_surv <- BC_chicks_Phi_run[[13]]$results$real
Chick_quadage_surv <- BC_chicks_Phi_run[[18]]$results$real
Chick_hatch_date_surv <- BC_chicks_Phi_run[[8]]$results$real
Chick_mgmt_s_surv <- BC_chicks_Phi_run[[15]]$results$real
Chick_mgmt_l_surv <- BC_chicks_Phi_run[[14]]$results$real
Chick_mgmt_s_x_quadage_surv <- BC_chicks_Phi_run[[20]]$results$real
Chick_mgmt_s_quadage_surv <- BC_chicks_Phi_run[[17]]$results$real
Chick_mgmt_l_quadage_surv <- BC_chicks_Phi_run[[16]]$results$real
...
# inspect Phi AIC table to find top model
BSC_chicks_Phi_run # best structure: Phi(~Year * Day_of_Season)
...
BSC_Chick_mgmt_l_surv <- BSC_chicks_Phi_run[[14]]$results$real
...
BSC_Chick_mgmt_l_quadage_surv <- BSC_chicks_Phi_run[[17]]$results$real # Top model, only one <2 delta 
...
BSC_Chick_dot_surv$mgmt_s <- unlist(substr(BSC_Chick_dot_surv$X2, 11, 11))
...
BSC_Chick_quadage_surv$mgmt_s <- unlist(substr(BSC_Chick_quadage_surv$X2, 11, 11))
...
BSC_Chick_hatch_date_surv$mgmt_s <- unlist(substr(BSC_Chick_hatch_date_surv$X2, 11, 11))
...
BSC_Chick_mgmt_s_surv$mgmt_s <- unlist(substr(BSC_Chick_mgmt_s_surv$X2, 11, 11))
...
BSC_Chick_mgmt_l_surv$mgmt_l <- unlist(substr(BSC_Chick_mgmt_l_surv$X2, 10, 10))
...
BSC_Chick_mgmt_s_quadage_surv$mgmt_s <- unlist(substr(BSC_Chick_mgmt_s_quadage_surv$X2, 11, 11))
...
BSC_Chick_mgmt_s_x_quadage_surv$mgmt_s <- unlist(substr(BSC_Chick_mgmt_s_x_quadage_surv$X2, 11, 11))
...
BSC_Chick_mgmt_l_quadage_surv$mgmt_l <- unlist(substr(BSC_Chick_mgmt_l_quadage_surv$X2, 10, 10))
...
# inspect Phi AIC table to find top model
FP_chicks_Phi_run # best structure: Phi(~Year * Day_of_Season)
...
FP_Chick_quadage_surv <- FP_chicks_Phi_run[[18]]$results$real
FP_Chick_hatch_date_surv <- FP_chicks_Phi_run[[8]]$results$real
FP_Chick_mgmt_s_surv <- FP_chicks_Phi_run[[15]]$results$real
FP_Chick_mgmt_l_surv <- FP_chicks_Phi_run[[14]]$results$real #3rd model
FP_Chick_mgmt_s_x_quadage_surv <- FP_chicks_Phi_run[[20]]$results$real
FP_Chick_mgmt_s_quadage_surv <- FP_chicks_Phi_run[[17]]$results$real
FP_Chick_mgmt_l_quadage_surv <- FP_chicks_Phi_run[[16]]$results$real
FP_Chick_hatch_date_mgmt_l_surv <- FP_chicks_Phi_run[[6]]$results$real # top model
FP_Chick_hatch_date_x_mgmt_l_surv <- FP_chicks_Phi_run[[10]]$results$real # second top model. only two models <2 Delta AIC
...
# interaction between hatch date and age and management level survival - top model
ggplot() +
  geom_line(
    data = filter(FP_Chick_hatch_date_surv, X1 == "Phi"),
    aes(x = date_j, y = estimate, color = mgmt_l)
  ) +
  geom_ribbon(
    data = filter(FP_Chick_hatch_date_surv, X1 == "Phi"),
    aes(x = date_j, ymin = lcl, ymax = ucl, fill = mgmt_l),
    alpha = 0.2
  ) +
  labs(
    y = "Daily brood survival probability (Φ)",
    x = "Hatch date",
...
PI_Chick_dot_surv <- PI_chicks_Phi_run[[13]]$results$real #3rd model
PI_Chick_quadage_surv <- PI_chicks_Phi_run[[18]]$results$real # 2nd model
PI_Chick_hatch_date_surv <- PI_chicks_Phi_run[[8]]$results$real # 4th model
...
# age-dependent survival Number 1 model
ggplot() +
