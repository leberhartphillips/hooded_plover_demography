...
summary(S.fundays_status) # use this to find summary statistics for models
...
MP_chicks_p_run$model.table %>%
   ...
   mutate(model = str_replace_all(model, c(
      "Quadratic" = "quadratic age",
       ...
       "Time" = "time",
      "season" = "year"))) %>%
  select(model, npar, DeltaAICc, weight) %>%
  gt() %>%
  cols_label(model = html("<i>Mornington Peninsula</i>"),
    npar = "K",
    DeltaAICc = "DeltaAICc",
...
ggplot(pred_df %>% 
  ...
  labs(x = "number of weekend days and holidays exposed to", 
       y = "apparent daily brood survival (± 95% CI)", color = "mgmt_l", fill = "mgmt_l") +
  theme_minimal() +
  facet_grid(. ~ mgmt_l)
```
...
# Extract estimates of survival from top model
Chick_p_reals <- BC_chicks_p_run[[3]]$results$real
...
# inspect p AIC table to find top model
BSC_chicks_p_run # best structure: p(~season + Quadratic)
...
#BSC_Chick_quadage_surv <- BSC_chicks_Phi_run[[16]]$results$real
...
#BSC_Chick_mgmt_s_quadage_surv <- BSC_chicks_Phi_run[[18]]$results$real
BSC_Chick_mgmt_l_quadage_surv <- BSC_chicks_Phi_run[[22]]$results$real # Top model
...
# inspect p AIC table to find top model
PI_chicks_p_run # best structure: p(~season + Quadratic)
