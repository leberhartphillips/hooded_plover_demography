...
### Fundays for broods
```{r}
# Mike and Jas attempt to calculate fundays
# need brood start and end date 
MP_chick_data <- MP_chick_data %>%
  mutate(
...
# Step 1: Clean and filter valid rows
valid_rows <- MP_chick_data_clean %>%
...
# Step 2: Generate date sequences per nest
fundays_brood_df <- pmap_dfr(valid_rows, function(nest_ID, hatch_date, last_checked, ...) {
  tibble(nest_ID = nest_ID, date = seq(hatch_date, last_checked, by = "day"))
}) %>%
  mutate(
    weekday = weekdays(date),
    region = "MP",
    year = year(date),
    season = ifelse(month(date) < 6, year - 1, year),
    season = ifelse(month(date) >= 6,
                    paste0(year, substr(year + 1, 3, 4)),
                    paste0(season, substr(year, 3, 4)))
  ) %>%
  left_join(holidays, by = c("region", "season")) %>%
  mutate(holiday = ifelse(date >= start_date & date <= end_date, 1, 0)) %>%
  group_by(region, season, date) %>%
  mutate(holiday = max(holiday, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(day_type = ifelse(holiday == 1 | weekday %in% c("Saturday", "Sunday"), "funday", "workday"),
         funday = ifelse(day_type == "funday", 1, 0))

# Step 3: Count fundays per nest
funday_counts <- fundays_brood_df %>%
  group_by(nest_ID) %>%
  summarise(funday_total = sum(funday, na.rm = TRUE))

# Step 4: Merge back into MP_chick_data_clean
MP_chick_data_clean <- MP_chick_data_clean %>%
  left_join(funday_counts, by = "nest_ID")



#code to use to add dates if we need
#mutate(hatch_date = ifelse(Fate == "0" & !is.na(clutch_comp), clutch_comp + 28, 
                             # ifelse(Fate == "0" & is.na(clutch_comp) & !is.na(chick_1st_ob) & !is.na(state_1st_ob),
                                   #  ifelse(state_1st_ob == "1/3 adult size, fluffy", chick_1st_ob - 3,
                                        #    ifelse(state_1st_ob == "1/2 adult size, fluffy", chick_1st_ob - 10,
